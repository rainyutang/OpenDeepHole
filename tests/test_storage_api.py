...
def test_authenticated_and_public_pages_keep_history_bounded(tmp_path, monkeypatch):
    ...
    for index in range(105):
        store._store_task_version_locked("s", {**task(index), "task_name": "candidate-audit-s-0" if index in {5, 57, 104} else "other"})
    store._conn.commit()
    ...
    async def run():
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
            for prefix, params in (("/api/v2/scans/s", {}), ("/api/public/scans/s", {"token": "public-test"})):
                ...
                assert [item["task_id"] for item in filtered["items"]] == [task(5)["task_id"], task(57)["task_id"]]
                last = (await client.get(prefix + "/tasks", params={**params, "task_name": "candidate-audit-s-0", "cursor": filtered["next_cursor"]})).json()
                assert [item["task_id"] for item in last["items"]] == [task(104)["task_id"]]
