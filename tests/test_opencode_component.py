...
def test_component_has_no_opendeephole_package_imports() -> None:
    component_dir = Path(opencode.__file__).resolve().parent
...
def test_component_directory_can_be_imported_as_an_extracted_package(tmp_path: Path) -> None:
    source = Path(opencode.__file__).resolve().parent
    target = tmp_path / "extracted_opencode"
    ...
    try:
        ...
        for name in list(sys.modules):
            if name == "extracted_opencode" or name.startswith("extracted_opencode."):
...
def test_configuration_is_lazy_and_shutdown_discards_both_singletons(tmp_path: Path) -> None:
    ...
    async def run() -> None:
        ...
        opencode.configure_opencode(_bindings(tmp_path))
        try:
