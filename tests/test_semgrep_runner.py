...
from deephole_client.vulnerability_mining.engines.static_candidate.static_analysis.semgrep_runner import (
...
def test_semgrep_runner_sets_noninteractive_env(tmp_path: Path) -> None:
    ...
    with patch("deephole_client.vulnerability_mining.engines.static_candidate.static_analysis.semgrep_runner.subprocess.run", side_effect=fake_run):
...
def test_semgrep_runner_prints_heartbeat_when_enabled(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("deephole_client.vulnerability_mining.engines.static_candidate.static_analysis.semgrep_runner.subprocess.Popen", side_effect=FakePopen),
        patch("deephole_client.vulnerability_mining.engines.static_candidate.static_analysis.semgrep_runner.time.monotonic", side_effect=[0.0, 0.0, 0.2, 0.2]),
    ):
        result = run_semgrep(
            tmp_path,
            rule_file=rule_file,
