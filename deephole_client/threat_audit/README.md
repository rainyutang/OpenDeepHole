...
| `on_task_result` | 否 | callable | 单个任务产出结果后的同步或异步回调 |
...
传入 `output` 后，过程会为每个派生任务发送
`process=threat_audit`、`kind=task_status` 的结构化事件。事件的
`data.task` 是完整任务快照，依次覆盖 `pending`、`running` 和最终的
`completed`、`failed`、`timeout`、`cancelled` 等状态，同时包含任务目标、
失败原因、模型输出来源和时间字段。`created_at` 在同一任务的整个生命周期保持稳定，
`updated_at` 随每次状态变化更新。

`on_task_result` 会在单个任务返回合法列表并完成漏洞规范化后立即收到
`task` 和 `vulnerabilities`；空列表也会回调一次。回调先于该任务的
`completed` 生命周期事件执行，过程仍会等待整批结束并返回完整汇总，独立调用方可同时
使用实时结果和最终结果。

漏洞挖掘框架中的内置威胁审计引擎会通过该回调逐条写入平台已有的漏洞接口，再发布带
`result_vuln_indexes` 的完成任务；因此其它威胁审计仍在执行时，“发现的问题”页面就能
收到结果。批次结束和扫描完成上报只补齐未成功流式上报的结果，不会重复写入已上报问题。
单条结果传输异常不会把已经成功的模型审计改成失败，缺少服务端索引的结果会在最终任务
写入前重试。普通独立调用方也可以直接消费事件和结果回调，而无需依赖平台模块。
