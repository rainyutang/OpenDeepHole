...
的 `attack_mode.json`。该目录是面向白盒代码审计整理的精简正交模式库，覆盖 C/C++ 内存与
整数安全、协议解析与状态机、密码学，以及系统与信任边界四类常见机制，不包含 Web 专属模式。
协议类进一步覆盖帧边界、字段语义、解析差异、分片重组、状态与顺序、身份与完整性、协商降级、
资源放大，以及认证通道、请求响应和消息处理上下文的绑定缺失。
其中拒绝服务分别覆盖致命终止、死锁/活锁、通用资源耗尽和协议资源放大，避免用一个宽泛模式
代替不同代码根因。
动态路径、续扫状态、产品 MCP 名称和攻击模式筛选提示写入
