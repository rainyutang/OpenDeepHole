...
from .models import ThreatAnalysis, ThreatAnalysisSources, ThreatAttackPath
from .parsing import (
...
class NoAvailableModelError(RuntimeError):
    """Raised when Task Agent cannot lease a model for a stage."""
...
class _OpenCodeRuntime:
    ...
    max_retries: int
    task_agent_config: str | Path | None
    output: Callable[[str], Any] | None
    cancel_event: Any
    mock: bool = False

    async def run_opencode_task(self, **kwargs: Any):
        return await run_opencode_task(
            **kwargs,
            config_path=self.task_agent_config,
            output=self.output,
...
def _detect_product_mcp(
    *,
    ...
    configured_mcp_names: list[str] | None = None,
    timeout_seconds: int = 60,
) -> dict[str, Any]:
    requested = str(product_mcp_name or "").strip()
    available_names = [
        str(name).strip()
        for name in configured_mcp_names or []
        if str(name).strip()
    ]
    result = {
...
async def run_attack_tree_threat_analysis(
    ...
    max_retries: int = _STAGE_FAILURE_RETRIES,
    task_agent_config: str | Path | None = None,
    ...
    capability = str(required_capability or "high").strip().lower()
    capability = "high" if capability in {"medium", "high"} else "low"
    opencode_runner = _OpenCodeRuntime(
    ...
    effective_timeout = timeout if timeout is not None else 1200
    analysis_root = (project_dir or workspace).resolve()
    target_path = (code_scan_path or analysis_root).resolve()
    run_dir = (work_dir or workspace / "threat_analysis").resolve()
    ...
    product_mcp_name = str(product_mcp_name or "").strip()
    product_mcp_detection_timeout = max(
        1,
        int(product_mcp_detection_timeout_seconds or 60),
    )
...
async def _invoke_stage(
    *,
    ...
    stage_retries = max(
        0,
        int(getattr(opencode_runner, "max_retries", _STAGE_FAILURE_RETRIES)),
    )
    ...
    for stage_attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            result = await task_runner(
                ...
                prompt=stage_prompt,
                required_capability=opencode_runner.required_capability,
            )
            if result.status == "timeout":
                raise asyncio.TimeoutError(result.text)
            if result.status == "failure":
                if "no available model" in str(result.text or "").lower():
...
def _stage_prompt(
    *,
    ...
    skill_file = Path(__file__).resolve().parent / "skills" / f"{skill_name}.md"
    if not skill_file.is_file():
        raise FileNotFoundError(
            f"Threat-analysis stage skill is missing: {skill_file}",
        )
    skill_content = skill_file.read_text(encoding="utf-8")
    prompt = (
        f"按下面内嵌的 `{skill_name}` 规则执行威胁分析阶段：{task_label}。\n"
        f"\n<stage-skill>\n{skill_content}\n</stage-skill>\n\n"
