"""C/C++ code-graph builder backed by Universal Ctags and tree-sitter.
...
from .source_filter import iter_source_files
...
class CppAnalyzer:
    def __init__(
        ...
        *,
        ...
        self.db = db
        self.ctags_executable = str(ctags_executable or "ctags")
    ...
    @staticmethod
    def _ensure_tools_available(ctags_executable: str = "ctags") -> None:
        if shutil.which(ctags_executable) is None:
            raise CodeIndexToolError(
        ...
        if not CppAnalyzer._ctags_supports_json_output(ctags_executable):
            raise CodeIndexToolError(
                "ctags 必须支持 JSON 输出。请安装带 JSON 输出支持的 Universal Ctags。"
            )

    @staticmethod
    def _ctags_supports_json_output(ctags_executable: str = "ctags") -> bool:
