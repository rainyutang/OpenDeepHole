...
同时启用威胁分析，并等待分析成功后再读取攻击树和高风险模块文件，仅按“叶子节点 + 关联
攻击模式”拆分任务。每个任务携带从叶子节点到价值资产的完整攻击路径、路径上模块去重后的
代码路径和目标资产说明；普通内部节点没有代码路径时省略。也可以显式不选任何漏洞挖掘引擎，
仅运行威胁分析。后端不执行这些过程，也不维护实现专属 Schema。
