...
set "MISSING_DEPS="
%PYTHON_CMD% -c "import semgrep, httpx, openai_codex, websockets, yaml, pydantic, tree_sitter, tree_sitter_cpp, uvicorn, fastapi; from importlib.metadata import version; from mcp.server.fastmcp import FastMCP; assert int(version('sse-starlette').split('.', 1)[0]) >= 3" 2>nul
