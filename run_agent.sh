...
if ! "$PYTHON_CMD" -c "import httpx, openai_codex, websockets, yaml, pydantic, tree_sitter, tree_sitter_cpp, uvicorn, fastapi; from importlib.metadata import version; from mcp.server.fastmcp import FastMCP; assert int(version('sse-starlette').split('.', 1)[0]) >= 3" 2>/dev/null || ! command -v semgrep >/dev/null 2>&1; then
