"""Shared structured-output contract for vulnerability audit tasks."""
...
THREAT_AUDIT_VULNERABILITY_ITEM_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        key: value
        for key, value in VULNERABILITY_ITEM_SCHEMA["properties"].items()
        if key != "confirmed"
    },
    "required": [
        key
        for key in VULNERABILITY_ITEM_SCHEMA["properties"]
        if key != "confirmed"
    ],
    "additionalProperties": False,
}
