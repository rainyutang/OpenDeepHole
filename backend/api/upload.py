...
async def upload_source(file: UploadFile, background_tasks: BackgroundTasks) -> UploadResponse:
    ...
    Accepts a .zip file, extracts it into the projects directory,
    and returns a project_id for subsequent scan requests.
    """
    ...
    logger.info("Uploaded project %s (%d bytes)", project_id, len(content))
