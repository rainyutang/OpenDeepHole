...
- **修复** checker 调用 OpenCode 后未提交结果时不再被当作「非问题」；该候选会标记为失败、保留最后一次 OpenCode 输出作为失败原因，并进入失败/未完成候选重跑流程
- **优化** 扫描详情页新增失败候选计数和失败原因展示，重跑按钮文案改为「重跑失败/未完成」

## 2026-06-27

- **新增** 扫描详情页问题列表支持按人工标记情况筛选，可与严重性、类型筛选叠加过滤未标记、确认正报、实为误报和待分析候选
- **变更** 扫描前内存申请/释放函数识别改为默认关闭，可通过 `memory_api_discovery.enabled: true` 在配置文件中开启；已有 `memory_api_pairs.json` 仍会被内存类 checker 复用
...
- **优化** AI 去误报手动触发改为先补跑未产生有效复核结论的候选；只有全部候选都完成一轮有效复核后，再次点击才会全量重跑，同时补跑期间保留页面已有复核结果
...
- **变更** 扫描详情页改为**左右主从布局**：左侧为精简问题列表（文件:行 / 函数 / 类型 / 严重级别 + AI/去误报状态徽章），顶部保留严重级别与类型筛选；右侧为选中问题详情，描述、AI 分析与去误报各阶段（正方论证、反方论证、最终裁决）输出均以 Markdown 渲染展示
