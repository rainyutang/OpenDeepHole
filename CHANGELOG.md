...
- **修复** Agent 启动及每次任务同步配置时不再用设置页的代理跳过列表覆盖系统已有 `NO_PROXY/no_proxy`：大小写变量现在分别保留各自的系统原值，并以逗号追加用户在客户端设置中配置的 `base.no_proxy`，重复启动不会反复累加，配置更新会替换旧的设置部分；OpenCode/nga Serve 启动环境显式使用同一合并结果
