...
- **新增** 验证方法可自行定义本地 `main()`，通过 `run_validator_debug(...)` 复用真实 `agent.yaml`、OpenCode/MCP 上下文和线上验证运行链并调用同一个 `validate(**kwargs)`；新增 `docs/vulnerability_validation.md` 说明完整参数、调试模板、运行命令和结果位置
