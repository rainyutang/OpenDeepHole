...
- **优化** `memleak` checker 改为 opencode 复核，并将静态阶段升级为 tree-sitter 路径分析：`if` then/else 分支单独传播，过滤判空宏、初始化失败、参数/参数成员所有权转移和循环内正常释放后的函数尾部 return 误报，同时保留循环 `continue` 泄漏候选
