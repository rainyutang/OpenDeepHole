...
- **优化** 扫描详情顶部流程图严格收敛为并列等高的“威胁分析 / 渗透执行”和下方通栏“基础能力”：威胁分析按纵向四个结果入口直达价值资产、高风险模块、内部节点和攻击树；渗透执行横向展示程序分析、完整引擎目录、独立问题按钮、不同视角分析及漏洞验证，并突出本次选中引擎；基础能力集中展示代码图谱构建状态、文件数及知识库启用状态
- **修复** 威胁分析产物已经上传时，扫描首页仍受滞后的运行状态和初始化请求竞态影响而显示“正在执行”、进入结果页后继续等待；有效产物现在立即作为完成判据，上传接口同步落库成功状态并按状态、产物顺序推送，四个结果入口随即开放
- **修复** 漏洞挖掘引擎通过 `report_vulnerabilities()` 上报的每个有效漏洞现在立即作为正式结果持久化、开放人工反馈，并按扫描配置逐条进入自动去误报和漏洞验证，不再等待全部引擎结束；扫描完成时仅对尚未形成有效结论的项目执行幂等补漏，单项失败继续与后续问题隔离
- **变更** 漏洞挖掘引擎 `run()` 返回值收敛为必填 `status` 及可选错误和进度字段，漏洞只能通过 `report_vulnerabilities()` 上报；旧引擎返回的 `vulnerabilities` 字段会被忽略并记录迁移告警，内置静态候选审计、威胁审计及三套扩展示例均已迁移且保留原有逐条上报、重试和任务索引行为
- **兼容** 新版 Agent 通过握手能力标记接管升级前已经落库的临时 callback 结果：按稳定 Agent 身份保留原漏洞索引并提升为正式结果，刷新扫描页面后立即允许人工反馈，welcome 后补发未完成去误报及已配置验证；旧 Agent 继续使用原临时批次对账协议，用户明确停止的任务不会被自动重启

## 2026-08-19

- **修复** 扫描级 CodeGraph MCP 首次连接失败或运行中连接关闭后，共享租约不再把缓存的不可用状态持续传给所有后续任务：每次复用前会核对 OpenCode 实时状态，失联时在目录级锁内受控重连并让并发任务共享恢复结果；瞬时失败经过有界重试和冷却后可由后续任务继续自愈，配置、名称或鉴权错误仍保持不可重试，恢复失败时继续使用文件工具且不盲目断开可能已经连接成功的实例
- **优化** 扫描详情移除右上角重复的“AI去误报”入口，启动、重跑与停止统一收敛到漏洞复核页；存在未形成有效 TP/FP 结论的项目时按钮显示“启动复核”并只补跑这些项目，全部成功后改为“重新复核”并全量重跑；续扫同步纳入所有未成功复核项，包括从未启动过复核或整轮完成但部分单项失败的扫描，已全部成功的复核不会因续扫而重跑
- **修复** 扫描历史接口读取轻量漏洞统计时未携带临时结果标记，导致复用可重试判定后触发 `VulnStat.provisional` 属性异常；统计投影现在保留真实临时状态，临时超时或失败结果也不会被误计为可续扫候选
- **修复** 漏洞挖掘引擎的 `report_vulnerabilities()` 结果改为按运行批次以只读临时项追加展示；当 `run()` 返回 `success` 且最终 `vulnerabilities` 非空时，以最终列表严格替换本次回调项，成功空列表、失败或取消时则保留回调并追加有效部分结果；权威列表重新执行扫描级去重并原子对账后才进入自动去误报和漏洞验证，完成接口保留失败重试兜底
- **修复** 仅提供 `vulnerability_report` 的引擎结果不再因定位和结构化证据为空而合并，完整报告字段会持久化并在扫描详情及公开分享页中直接渲染；临时项在最终对账前禁止人工标记和手动验证
- **优化** 扫描详情侧边栏的“执行流程”不再重复展示代码图谱构建入口，“辅助工具”更名为“其它信息”并新增扫描信息抽屉；抽屉集中展示创建扫描时的名称、路径、Agent、产品、扫描模式、知识库与 CodeGraph MCP 开关，以及威胁分析、漏洞挖掘、自动去误报和漏洞验证配置，选择基于静态分析的引擎时同时列出实际生效的静态规则快照；公开分享页面同步展示完整扫描路径，但不暴露 MCP 连接参数等私密配置
- **优化** 扫描详情将报告导出统一到顶部菜单，在所有扫描状态下均可导出当前已保存结果的 Markdown 压缩包或完整 CSV；“发现的问题”页不再单独提供受筛选条件影响的 CSV 下载按钮

## 2026-08-18

- **优化** 扫描详情新增桌面常驻、窄屏抽屉式左侧导航，统一收纳首页、发现的问题、执行阶段、SKILL 预览、模型看板、误报屏蔽规则、SKILL 报告和日志入口；顶部流程图及各详情页面保持不变，并与侧栏共用阶段名称、状态和引擎选择逻辑
- **新增** 新建扫描改为快速、标准、自定义三种模式并默认选择快速模式：快速和标准固定启用两个内置漏洞挖掘引擎、威胁分析/威胁审计和自动去误报，自定义模式保留原有开关、方法与引擎选择；服务端固化模式及实际执行快照，并拒绝固定模式下相互矛盾的覆盖参数
- **新增** Agent 受管配置升级为 v7，高级配置可分别维护三种扫描模式下“基于代码风险点”引擎的规则；快速模式默认排除敏感信息和测试规则，标准模式默认只排除测试规则，自定义模式沿用原全局排除项，未禁用且当前可用的新规则继续自动启用，`enabled: false` 规则仍不进入线上清单
- **变更** `scan_mode` 统一进入代码图谱、威胁分析外层适配器、漏洞挖掘引擎及其内置过程、去误报方法、漏洞验证器的 `kwargs` 和 Task Agent 任务元数据；原生威胁分析五参数入口保持不变，历史 `full` / `threat_analysis_only` 扫描续跑时在组件边界映射为 `custom`
...
- **修复** Windows Agent 重启后 ownership marker 中的旧监听 PID 已不存在、目标端口也未实际监听，但 `netstat` 仍返回陈旧 PID 时会反复执行失败的 `taskkill`、永久保留 marker 并阻断所有模型任务：回收现在以 TCP 连接结果和 `SO_EXCLUSIVEADDRUSE` 独占绑定共同确认端口是否可复用，可绑定时忽略幽灵监听记录并清除 marker，不可绑定的真实受管残留仍继续终止并在失败时阻止启动；自动端口遇其它进程占用会在不终止对方的前提下最多尝试 3 个不同端口，且旧受管 Serve 清理未完成时禁止换号，同一 Agent 进程不会同时登记或启动多个 Serve
- **修复** “DeepHole基于攻击威胁的漏洞挖掘引擎”不再因单条攻击路径包含空 ID、未知节点、重复节点或节点类型顺序异常而在任务入队前终止整批扫描；威胁审计现在跳过坏节点并按 `node_ids` 顺序保留从叶子节点向根节点追溯的最长可用链路，未到根节点也继续创建任务，完全不可恢复的路径仅记录包含树、路径、节点及原因的告警，同树其它路径和其它攻击树继续执行，跨树任务隔离及 Prompt/漏洞 JSON 契约保持不变
...
- **重构** 威胁审计任务收敛为攻击树中每个“叶子节点 + 关联攻击模式”只创建一次；Prompt 只保留攻击模式、叶子节点到价值资产的完整路径、路径上按规范化路径全局去重的模块代码路径和目标资产说明，代码路径统一展示为 `模块名 : /路径，描述`，并明确这些信息仅作线索、仍须验证外部入口到漏洞触发点的真实代码链路，输出 JSON 契约保持不变
- **兼容** 同一叶子节点异常出现在多条历史攻击路径时临时合并上下文并输出告警；旧版尚未完成的内部节点和根节点威胁审计任务标记为“已取代”，保留历史但不再续跑，也不计入当前任务进度、失败和完成判断，该兼容分支后续可独立移除
- **优化** 首页右上角公告按钮在最近已发布公告新增、重新发布或内容更新后显示未读圆点；用户点击公告后按账号在当前浏览器记录已读状态，刷新或重新登录不再重复提示，标签页重新可见时自动检查更新
- **新增** 所有漏洞挖掘引擎共用扫描级前置去重：已确认问题先按规范化项目相对完整路径和行号严格去重，再对同文件同函数的历史已接受报告启动一次根因 LLM 裁决；同根因结果在持久化、SSE、自动去误报和漏洞验证前丢弃，不同引擎并发上报时保持先到结果
- **容错** 续扫会分页加载已有漏洞作为去重基线，避免重跑引擎再次保存同一问题；模型不可用、超时、失败或 JSON 无效时记录去重错误并放行当前结果，模型任务在扫描队列中单独显示为“漏洞去重”且不干扰候选点审计状态

## 2026-08-11

- **修复** 停止扫描后候选点审计和威胁审计不再让整批已排队协程依次穿过并发限制、打印审计日志或逐条上传取消状态；两个过程改为有界 worker 按需取任务，取消后未启动任务保持可续扫，威胁审计取消批次也不再重放最终任务列表，上传失败日志补充扫描、任务、状态及异常类型
- **修复** 候选点审计总数固定为静态分析产生的完整候选集合，定向续扫不再用剩余子集覆盖总量；客户端随处理键上报绝对完成数，服务端按持久化候选数、引擎生命周期和单调上界统一校验，运行中始终小于总数、引擎成功时精确等于总数，流程图不再保留或展示“已完成大于总数”的旧快照
- **变更** 新建扫描的知识库配置收敛为启用开关、项目拉取和项目选择；远程 URL、请求头、超时以及 `projects_tool` / `set_project_tool` 名称统一放入服务端 `config.yaml`，页面和创建扫描接口不再接受连接参数，并按用户与稳定客户端只记忆上次选择的项目 ID 和名称
- **安全** 扫描启用知识库时，受管 OpenCode 插件在每次知识库查询工具真正执行前原地强制覆盖 `project_id` 为扫描所选 ID，无论模型省略或传入其它值；项目列表与切换工具从模型工具集中隐藏并在 Hook 中二次拒绝，平台不调用 `set_project`，也不向 Session 历史或系统提示写入绑定消息
- **容错** 每个 OpenCode Session 在首个业务 prompt 前完成知识库连接、工具发现和私有项目绑定；任一步失败都会断开并禁用该知识库 MCP 的工具，同时继续原业务任务，不把知识库故障升级为扫描失败
- **新增** 扫描详情顶部流程图的“底层能力”框始终显示本次扫描的 CodeGraph MCP 与知识库启用状态；状态来自扫描创建时固化的私有配置快照，不暴露连接参数，也不把运行时连接失败误写为未启用
- **优化** 扫描历史移除任务进度和检查项，项目名称改为用户内唯一的扫描名称，列表改为展示尚待人工查看的疑似问题与人工确认漏洞；主扫描完成后去误报仍在执行时继续显示进行中，统一停止和续扫会同时处理主扫描与未完成去误报任务，并只重排尚无结论的去误报问题
- **修复** `run_opencode_task()` 将 `file_write_allowlist` 与兼容参数 `writable_paths` 统一为额外可写且默认保留的路径，支持单路径和目录后代，`work_dir` 始终隐式可写并进入白名单；所有调用都会清理本轮在白名单外任意位置新建的文件，而实际采用的文件型 JSON 结果会在解析后强制删除，既有文件保持不变
- **优化** 对抗式去误报改为两个动态补位槽，新增漏洞可在首项尚未结束时立即排队执行；各阶段只通过结构化上下文传递 Markdown，不再提示模型在项目目录创建 `prove_bug.md`、`prove_fp.md` 或最终裁决文件

## 2026-08-10

- **修复** Windows 下受管 OpenCode Serve 启动器与实际监听进程分离后，端口扫描已确认监听 PID、但通用 PID 探测返回假阴性时可能只打印回收日志而未执行 `taskkill`，继而保留 ownership marker 并让后续模型任务连续启动失败：受管监听现在以目标端口上的实际监听状态驱动 `taskkill /T /F`、直接终止兜底和最终验证；清理失败会保留完整的内存归属状态供下一次重试，未知监听者仍不会被终止
...
- **修复** `run_opencode_task()` 的文件 JSON 回退增加受管 OpenCode 写入 Hook，并在消息结束后按发送前基线重放本轮完整 assistant 历史，能够识别最终响应未携带的中间 `write` / `edit` / `apply_patch` 写入；相对路径按项目目录解析，只读取受信任可写范围，并只自动清理任务工作目录中新建的临时文件
- **变更** 结构化结果不合规时先以 `required_capability=low` 从当前已启用模型中优先选择最低能力候选，创建禁用工具的独立格式匹配 Session，只允许在不改变内容的前提下修复格式；原文与 Schema 无关或不能无损映射时返回固定非法值，随后才在原业务 Session 追加纠正，仍失败再进入 fresh 业务 Session 重试，格式匹配成功时继续保留原业务 Session、文本和模型
- **修复** 自动去误报不再因某个单项失败或 Agent 侧意外取消而停止处理后续问题：每个新确认问题继续独立入队，普通失败仅保留为可补跑的未完成项，扫描结束或手动补跑时再次处理；只有用户明确点击停止后，扫描级去误报任务才保持取消并阻止继续自动入队
- **修复** 手动更新客户端不再被 Agent 模型池中停止任务遗留的 `running` / `queued` 快照永久阻塞，是否可更新改以服务端持久化的扫描、去误报和验证生命周期为准；全部任务终止后即可下发更新，真实活动任务仍受保护，新建扫描和续扫继续在任务执行前自动同步客户端运行时
- **修复** `run_opencode_task()` 在 Windows 上被可执行文件旁应用级 `config.json` 污染时生成无效 OpenCode 配置：自动发现的可执行文件相邻目录和项目目录现在只接受 `opencode.json` / `opencode.jsonc`，不再把安装器或启动器的 `env`、`version` 等字段写入受管 workspace；创建或更新 Session 返回 HTTP 5xx 时会把共享 Serve 标记为异常，等待在途 Session 退出后由下一次重试只重启一次，避免连续三次复用同一异常进程
- **变更** 两个内置漏洞挖掘引擎分别更名为“DeepHole基于攻击威胁的漏洞挖掘引擎”和“DeepHole基于代码风险点的漏洞挖掘引擎”，历史扫描、运行记录和漏洞结果按稳定引擎 ID 统一名称；扫描详情主流程调整为“威胁分析 → 漏洞挖掘 → 漏洞验证”，去误报从漏洞挖掘后并列分出，静态分析归入代码风险点引擎内部展示，实际并行与增量执行语义保持不变
- **修复** 首页扫描历史状态列保持单行展示，表格空间受限时不再将“审计中”“已取消”“已完成”等状态徽标拆行，窄屏继续通过表格横向滚动查看
- **重构** 客户端工具语义统一为 `opencode`，Web 基础配置只保留 OpenCode，并由全局 executable 单独指定 `opencode`、`nga` 或完整路径；移除模型行工具/可执行文件覆盖，受管配置升级为 v6、独立 `task-agent.yaml` 升级为 v2，旧 `nga` 配置迁移时继续启动 `nga` 且兼容发现 `~/.config/nga`
- **修复** 失败扫描续扫改为按阶段恢复：威胁分析先以 `is_resume=true` 复用已有阶段产物，返回失败且未取消时由外层协调器归档旧产物并仅做一次 `is_resume=false` 的干净回退；已成功的漏洞挖掘引擎不会重复执行，只有依赖威胁分析的 `threat_audit` 会随其补跑
- **修复** Agent 续扫不再复用旧任务的取消事件；旧任务会先完成协作式取消和退出，并抑制其过期终态覆盖服务端已进入待续扫的状态，新任务使用独立事件，任务清理也会校验对象身份，避免旧任务收尾误取消或移除刚创建的续扫任务
- **文档** 补充威胁分析续扫、单次干净回退、失败产物归档和 `retry_mining_engine_ids` 定向引擎重试契约；恢复策略全部位于平台协调层，不修改内置原生威胁分析方法

## 2026-08-05

- **修复** Windows 客户端首次去误报等模型任务偶发 `OpenCode serve` 以 `Error: Unexpected error` 退出：Agent 现在先按受控优先级合并全局、可执行文件目录、项目和显式指定的 OpenCode 配置，再以隔离的 `XDG_CONFIG_HOME` 启动 Serve，避免子进程二次读取用户配置；自动端口遇占用、系统保留/安全软件拒绝或无明确原因的首次早退会有界换端口恢复，显式端口保持不变且给出可操作诊断；全程不终止未证明属于本 Agent 的监听进程，启动日志会脱敏并明确未设置本地 Server 密码的警告不是退出原因
- **优化** 新建扫描页先集中填写扫描名称、项目总路径、代码扫描路径和产品，再配置扫描能力；产品支持自由输入和验证方法建议，扫描历史不再修改旧的产品/验证环境组合
- **重构** 静态分析与候选点审计 Checker 移入客户端全局配置，内置和用户 Checker 默认全选且以后新增项自动启用；创建扫描时固化当前选择，修改后从下一次扫描生效
- **变更** 代码图谱 MCP 创建配置收敛为启用开关与本地/远端选择：本地固定使用现有默认参数，远端只接受 URL 和请求头；服务端统一规范化名称、超时和本地命令
- **新增** 原全局产品信息 MCP 改为扫描级「启用知识库」，仅支持远程 URL 和请求头、默认关闭，并按当前用户和稳定客户端记忆上一次成功创建扫描时的配置；知识库与代码图谱可在同一任务会话独立加载和释放
- **重构** 漏洞验证的漏洞类型、并发、整体重试和模型策略继续在客户端全局配置中维护；新建扫描只选择是否启用、兼容产品的方法及其 field 参数，配置按用户、客户端、产品和方法记忆，并与全局策略一起固化为扫描快照
- **变更** 验证方法 YAML 收敛为严格的 `label`、`description`、`product`、`field` 四字段格式，方法 ID 直接使用目录名；旧验证环境、注册项和旧字段格式不再加载，v4 多环境策略升级时重置为新的通用默认值
...
- **文档** 说明威胁分析方法的固定五参数 `run_threat_analysis` 入口、成功/失败返回、三份 JSON 产物结构，以及从 `ThreatAnalysis/src/threat_analysis_harness` 直接复制覆盖内置方法的兼容流程

## 2026-08-04

- **新增** 结果看板向所有登录用户开放：管理员继续查看全部扫描，普通用户仅汇总自己创建的扫描；看板顶部新增当前权限范围的 Token 总计与按 Agent 明细，统计不受产品筛选影响，扫描详情继续保留单次扫描和任务级用量
- **重构** Web「Agent 配置」更名为「客户端配置」，子页收敛为「基础配置」和「高级配置」；基础页集中基础参数与模型配置，高级页以单开折叠区管理威胁分析、产品信息、漏洞挖掘、去误报和漏洞验证
- **变更** 新注册客户端的模型池固定为空，已有稳定客户端重连时保留原配置；Web、后端、续扫和外部集成入口统一要求至少一个已启用的明确模型，未配置时不再允许创建或继续扫描
- **移除** 客户端配置页的 Token 面板、Web 自定义 OpenCode JSONC 配置栏和运行配置查看器；Agent RPC、服务端运行快照与诊断模型同步删除，SQLite/PostgreSQL 升级时清理旧 `opencode_config` 数据并删除快照列，内部受管 OpenCode、MCP、SKILL、权限和独立 Task Agent 配置保持不变
...
- **修复** 登录鉴权、普通用户扫描历史/详情及 Agent 高频回调统一移出 FastAPI 事件循环；密码校验、Agent 运行包压缩和同步存储等待不再阻塞 WebSocket 心跳，新增请求 ID、Server-Timing、慢请求/慢存储日志、事件循环延迟与 SSE 队列运行指标
- **优化** 扫描历史改为稳定游标分页，详情拆分为 overview、候选点、漏洞、事件、威胁审计任务和验证记录等有界接口；前端首屏只加载 50 条历史及各详情首批数据，并支持继续加载，旧 API 保留兼容
- **新增** Agent 协议 v2 支持候选点、事件和已处理 key 分批上报、轻量结束报文及短期 HTTP 恢复清单；v1 保留一个发布周期，批次重试和漏洞结束兜底避免断联或实时上传失败时丢失结果
- **新增** 生产存储支持 PostgreSQL 16 与默认 4 Worker，Agent 会话/命令/RPC/SSE 使用持久化跨 Worker 协调、LISTEN/NOTIFY 唤醒和 advisory leader；SQLite 继续限定单 Worker，并提供只读快照迁移脚本、真实 PostgreSQL 集成测试及生产部署/回滚文档
- **文档** 新增统一的框架开发指南，说明 Agent 扫描整体流程、八个业务过程、框架向组件提供的目录/模型/事件/取消能力，以及漏洞挖掘引擎、静态 Checker、产品验证器和独立过程的接入与返回契约
- **重构** 静态分析与候选点审计实现归入 `vulnerability_mining/engines/static_candidate`，静态代码和审计 Skill 按规则合并到统一目录；新规则及嵌套 `skills/<name>/SKILL.md` 可自动发现，`checker.yaml` 配置保持不变，旧顶层过程包和用户平铺 Skill 格式不再加载
- **优化** 代码图谱构建按阶段只在开始、跨越 10% 进度档位和结束时上报结构化索引状态，不再把同一数值进度重复写入扫描事件日志
- **优化** OpenCode Task、Session、Tool、SKILL 等模型执行明细仅保留在 Agent 控制台，后端兼容过滤旧 Agent 上传的明细并自动清理历史记录；每个扫描的事件日志统一只保留最近 200 条，验证器显式发布的过程输出不受影响
- **重构** 威胁分析从 `threat_audit` 漏洞挖掘引擎中拆为扫描级独立过程，新增独立选择与生命周期；可以仅运行威胁分析，选择威胁审计时则强制同时启用威胁分析并等待其成功产物
- **优化** 新建扫描分别配置威胁分析和漏洞挖掘引擎；扫描详情固定展示静态分析、威胁分析、漏洞挖掘、漏洞验证和去误报，并把代码图谱构建作为支撑全流程的底层能力单独显示，未选阶段明确标记为已跳过
- **兼容** SQLite 自动为旧扫描推导威胁分析开关并固化历史默认引擎快照；续扫保留显式空引擎列表，避免仅威胁分析扫描回退到默认全引擎
- **修复** 普通候选点审计不再采用模型自由生成的漏洞类型，确认问题统一使用静态分析候选携带的类型，避免同类结果因模型措辞不同被拆成多个筛选项；项目级审计、威胁审计及其 JSON Schema 保持不变

## 2026-07-31

- **修复** Agent WebSocket 单消息接收上限由库默认 1 MiB 提升为可配置的 64 MiB，避免大代码仓续扫携带较多候选点时触发 1009 断连和反复重连；超限日志会提示通过 `OPENDEEPHOLE_WS_MAX_MESSAGE_MB` 调整
...
- **变更** 引擎漏洞元素只强制提供非空 `vulnerability_report`，其它定位、分类和证据字段全部可选，缺失时页面明确显示未知；`confirmed` 默认视为问题，引擎归属和人工反馈字段继续由平台可信状态覆盖
- **文档** 漏洞验证输入明确只保证报告非空，文件、函数、类型和调用链允许缺失；验证方法清单声明的全部用户配置键始终注入 `validate(**kwargs)`，必填缺失时报错，可选无默认值时传入 `None`
- **重构** `run_opencode_task()` 将候选点审计、项目级审计和威胁审计统一为 `task_type=vulnerability_mining`，旧审计类型不再接受；模型看板改按稳定 `task_name` 前缀区分具体漏洞挖掘子任务，历史任务快照继续兼容展示，公共指导只保留漏洞挖掘、威胁分析、去误报和漏洞验证四类
- **重构** 漏洞挖掘引擎的 `engine.yaml` 收敛为必填的 `label`、`description`、`fp_review` 三个字段，引擎 ID 直接使用目录名；旧版清单字段不再接受，新建扫描默认选择代码仓中的全部有效引擎
- **变更** `fp_review` 仅在新建扫描时只读提示引擎“自带去误报”或“不带去误报”，不再影响平台流程；所有引擎及历史扫描的问题统一支持平台自动和手动去误报，详情流程图始终展示去误报阶段
- **兼容** SQLite 保留旧 `fp_review_eligible` 物理列但不再向模型、API 或前端暴露，历史值不会过滤去误报候选，避免为本次契约变更执行破坏性迁移

## 2026-07-30

- **修复** 威胁审计每个任务产出确认问题后立即逐条持久化并推送“发现的问题”，不再等待整批威胁审计或整个扫描结束；完成任务同步记录漏洞索引，批次汇总继续兜底且不会重复上报
- **修复** 长时间静态规则 checker 改在后台线程执行，避免阻塞 Agent WebSocket 心跳并被后端误判离线；Agent 控制台和扫描详情日志恢复逐规则阶段及限流进度，checker 数与候选数不再覆盖真实文件统计
- **优化** “敏感信息未清理”审计 Skill 补充认证凭据、密钥及随机材料的识别流程，按真实生命周期检查显式清零，并完善短期票据、安全字符串类型、测试代码和非敏感值的误报排除口径
- **优化** `run_opencode_task()` 的 fresh Session 重试优先切换到本次任务尚未尝试的合格模型，替代模型并发繁忙时继续等待；只有单模型可用或所有合格模型都已尝试后才按当前权重回退，JSON 纠错耗尽也会换模但不触发健康降权
- **新增** 模型池按模型消息请求失败或超时维护进程内健康等级，配置权重保持为基础权重，有效权重按最多四级半衰并以 10% 为下限；一次成功或连续 10 分钟无新请求故障恢复一级，Agent 重启后重置，JSON、上下文/输出长度、Serve/MCP/回调、取消和无模型错误不参与降权
- **优化** 登录首页的更新公告由扫描历史上方常驻栏改为页头右上角按钮，点击后弹窗查看最近公告；管理员继续从查看弹窗进入公告管理
- **文档** 漏洞挖掘引擎扩展新增两套完整可复制示例：一套注册相邻 Skill 并通过 `run_opencode_task()` 执行项目级审计，另一套以无 shell 的 argv 方式运行外部命令并转换 stdout JSON；示例覆盖结构化结果、错误和取消处理，默认不进入平台引擎目录
- **重构** Trail of Bits fp-check 改为确认问题后立即逐项复核，单项最多并发四个且继续受模型池约束；扫描完成后再独立执行跨漏洞攻击链检查与批次汇总，汇总不再改写单项 TP/FP 或问题数量
- **新增** fp-check 单项结果与批次汇总分别持久化和展示；单项重跑会把旧汇总标记为待更新，汇总生成失败或取消时保留上一次成功内容，并支持只重试汇总
- **修复** fp-check 阶段结构化输出全部失败时不再以 `complete` 和 0 条结果假完成；Agent 会输出 `JSON_RETRY`/失败生命周期，页面显示失败原因和已结束进度，未完成单项可再次补跑
- **优化** CSV 报告导出在审计结论 `confirmed` 外新增去误报结论 `fp_confirmed`；有效 TP/FP 分别导出为 `True`/`False`，未复核或结论不确定时留空
- **重构** 扫描详情流程图改为按本次扫描选择的漏洞挖掘引擎动态生成，并实时显示各引擎等待、执行、完成、警告、失败、取消或跳过状态；威胁引擎拆分展示“威胁分析 → 威胁审计”，去误报节点仅在至少一个已选引擎允许复核时出现，历史扫描保持兼容
- **新增** 扩展引擎可直接复用通用详情页查看运行状态、错误、耗时及该引擎输出的问题，无需新增前端路由
- **重构** 威胁审计从威胁分析结果页拆为独立任务过程页，支持全部任务的状态筛选、分页和详情查看，并展示 Prompt、Session、模型、Token、结果索引与失败原因；任务生命周期会在等待、运行和终态时实时持久化到平台
- **优化** 候选点、项目级和威胁审计的输出 Prompt 改为用最小完整真实源码直观展示外部输入、关键 Guard、数据传播和危险操作，并在根因中串联可控输入、调用链、Guard 有效性与可利用原因；有效防护会明确输出为非问题或不生成威胁漏洞

## 2026-07-29

- **新增** 客户端下载页可按 Agent 提交运行时更新；请求会持久化等待当前扫描、续扫、去误报、漏洞验证及模型队列全部空闲后再下发，等待期间新任务继续正常执行，更新交接阶段短暂阻止新任务并在目标版本重连后自动恢复
- **新增** 登录首页增加更新公告栏并预置少量近期更新内容；管理员可在首页弹窗中新建、编辑、发布、下线和删除纯文本公告，普通用户仅查看最近三条已发布公告
- **重构** 漏洞挖掘阶段升级为目录发现式引擎框架：每个引擎通过独立 `engine.yaml` 与 `engine.py` 提供平台输入转换、引擎执行和统一输出转换，现有“静态规则扫描 + 候选点审计”及“威胁分析 + 威胁审计”作为内置适配器并发运行；单引擎失败隔离，至少一个成功时扫描带警告完成
- **新增** 新建扫描直接读取当前代码仓的引擎清单，可选择一个或多个引擎并分别控制去误报资格，不依赖 Agent 上报或 Agent 默认配置；解析结果随扫描固化，漏洞结果、详情、筛选和导出显示来源引擎，禁用去误报的引擎结果不会进入自动或手动复核，也不显示相关按钮或状态
- **文档** 新增漏洞挖掘引擎扩展契约，说明目录清单、三个异步适配函数、平台上下文、统一输出、配置优先级、结果来源和故障隔离语义
- **新增** 新建扫描可分别选择是否自动去误报及复核方法：原流程命名为“对抗式复核”（`adversarial`）并保持行为不变，新增“Trail of Bits fp-check 复核”（`fp_check`）；选择会随扫描持久化，手动重跑继续使用创建时的方法
- **新增** Trail of Bits fp-check 复核来源于 Trail of Bits Skills Marketplace 的 `fp-check`，按其批量流程执行步骤 0、标准优先路由、数据流/可利用性/影响/PoC/反方审查/六道门依赖波次和最终攻击链检查；完整 Skill、六份参考资料和三份分析指南已翻译为中文，技术失败只保留阶段证据而不生成 TP/FP
- **新增** 扫描详情按方法动态展示名称、阶段与批次汇总，报告压缩包会附带 Trail of Bits fp-check 复核汇总；扫描创建 API 新增 `auto_fp_review` 和 `fp_review_method`
- **修复** 新建扫描页“自动去误报”开关为轨道和滑块设置固定尺寸与明确定位，避免开关状态切换时滑块与轨道错位
- **修复** 仅威胁分析模式会把威胁审计确认的漏洞并入扫描最终结果；完成上报可幂等补齐逐条上传遗漏的威胁漏洞并实时推送页面，不再因已有静态或部分结果而丢失
- **变更** 扫描详情“发现的问题”默认同时展示有效问题和去误报最终判定的 FP 非问题；FP 记录明确标记为“非问题”且不计入问题数量，超时、失败、无结果等其它记录仍通过“显示全部”查看
- **重构** 候选点/项目级与威胁审计的 JSON Schema 和输出指令从通用 `task_agent` 拆回各自的 `deephole_client` 进程目录；两个审计过程继续支持独立抽取，输出契约和运行行为不变

## 2026-07-28

- **变更** OpenCode 模型任务按固定类型优先级调度：漏洞验证 `90`、威胁分析 `75`、SKILL 创建 `70`、对抗式去误报 `60`、威胁审计与静态候选点审计 `50`；数值越大越优先，同优先级继续按 FIFO 执行且不抢占运行中任务
- **新增** `run_opencode_task()` 增加可选 `writable_paths`，可按文件或目录为当前 Session 额外开放 `read`、`external_directory` 和 `edit`；相对路径以 `project_dir` 为基准，显式空列表可清除 Session 覆盖，稳定 SKILL/工作目录权限和 `bash` 禁用仍由全局 `opencode.json` 管理
- **变更** 新建扫描的代码图谱 MCP 改为默认关闭的可选项；缺省、`null` 或禁用配置统一只使用文件工具，历史扫描、续扫、去误报和漏洞验证不再回退内置 `deephole-code`。Agent 同时删除该内置 MCP 的自动启动、项目注册和受管 OpenCode 配置注入，保留 `mcp_server/` 供独立手动启动，全局 `product_info` MCP 继续按目录热加载
- **重构** 威胁审计按攻击路径内的“节点 + 攻击模式”创建任务；同一组合跨路径只审计一次并合并相关代码目录、对外暴露面、资产、路径、模块和边关系，Prompt 上下文仅使用威胁分析原生产物中的已有信息
- **变更** 威胁审计模型输出改为不含 `confirmed` 的已确认漏洞裸列表，未发现漏洞返回 `[]` 并正常完成任务；平台汇总时统一补充确认状态，JSON Schema 解析同时避免把非法结果中的嵌套空数组误判为顶层空列表
...
- **重构** 候选点、项目级和威胁审计统一结构化漏洞字段：是否问题、四档严重程度、漏洞位置、类型、CIA 影响、完整相关代码、入口到触发点调用链、攻击入口、根因和触发条件；候选点单次返回对象，项目级与威胁审计返回可包含多个问题的裸列表
- **变更** 审计模型不再生成 `ai_analysis`、`ai_verdict`、`vulnerability_report` 或 `markdown_reports`；非问题固定为 `low`，内部判定状态由 `confirmed` 派生，旧扫描字段继续兼容读取
- **新增** 结构化漏洞字段贯穿 Agent 上报、SQLite、API 和扫描详情，下载报告及漏洞验证输入由平台按统一中文章节生成
- **优化** 候选点审计 Prompt 精简为 Skill、文件、行号、函数、相关变量和问题类型，不再重复附加候选线索；启用产品知识 MCP 时要求结合产品知识判断外部入口，所有调用链必须从外部入口开始
- **变更** 审计调用链节点改为包含函数名、文件路径和函数定义起始行的结构化对象，并贯穿存储、API、报告、验证与前端；历史字符串调用链继续兼容
...
- **新增** 扫描详情展示扫描累计 Token、实际 `provider/model` 明细和单任务用量；Agent 配置新增“Token 统计”，按稳定 Agent 身份跨进程累计，Agent 离线后仍可查看
- **兼容** Token 采集异常不会中断模型任务，只将统计标记为可能不完整；升级前的历史扫描保持“暂无统计”且不回填为零

## 2026-07-26

- **新增** Web 全部现用页面支持深色与浅色模式切换，首次默认深色并持久化用户选择；登录、注册、公开扫描及登录后页面共用统一主题，刷新首屏不闪烁，原生控件、滚动条和威胁分析视图同步适配
- **优化** 扫描详情顶部操作区支持响应式换行，执行流程图压缩为居中的紧凑布局；窄屏仅在流程容器内滚动，并以连续连线明确展示“候选点审计”同时进入“漏洞验证”和“对抗式去误报”，原有节点状态与点击导航保持不变
- **优化** 扫描历史、用户管理、结果看板、Agent 配置、SKILL 市场、误报规则抽屉和漏洞详情统一页面留白、表格滚动、移动端布局、焦点与 ARIA 语义；不改变 API、扫描流程或表单提交逻辑

## 2026-07-25

- **重构** 扫描详情的威胁分析结果页对齐原生 `ThreatAnalysis/web` 查看器，使用完整字段表格展示价值资产、高风险模块和内部节点，并以深色树状连线展示攻击树及叶子节点匹配的攻击模式；威胁审计任务、原生产物路径和运行日志继续保留
- **文档** 产品漏洞验证方法 README 补充 `validator.yaml` 编写说明，覆盖多产品/环境注册、动态字段类型、默认值、范围约束、Agent 配置注入方式和常见加载错误
- **修复** 静态分析完成后，候选审计和并行威胁审计不再通过 `output=None` 或嵌套上下文清空 Agent 控制台回调；`run_opencode_task()` 的 Session、Tool、MCP 与 SKILL 调用日志会持续输出，`read`/`write` 显示文件路径、`grep` 显示查询参数，当前扫描选中的代码图谱 MCP 额外显示完整单行输入参数且仍隐藏模型正文、推理和工具返回
- **优化** 候选点审计不再把完整 `SKILL.md` 拼进 Prompt，改为从 frontmatter 读取 Skill 名称并通过首行 `/<skill-name>` 按需加载；checker 规则目录按本次候选审计任务注册，不污染其它过程
- **修复** 候选点审计在每个模型任务完成后立即上报该候选的漏洞、报告和处理进度，扫描详情无需等待整批候选结束即可显示结果，并避免批次收尾时重复上报
