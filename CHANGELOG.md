...
- **新增** 新建扫描可分别选择是否自动去误报及复核方法：原流程命名为“对抗式复核”（`adversarial`）并保持行为不变，新增“证据门禁复核”（`fp_check`）；选择会随扫描持久化，手动重跑继续使用创建时的方法
- **新增** 证据门禁复核按 fp-check 批量流程执行步骤 0、标准优先路由、数据流/可利用性/影响/PoC/反方审查/六道门依赖波次和最终攻击链检查；完整 Skill、六份参考资料和三份分析指南已翻译为中文，技术失败只保留阶段证据而不生成 TP/FP
- **新增** 扫描详情按方法动态展示名称、阶段与批次汇总，报告压缩包会附带证据门禁复核汇总；扫描创建 API 新增 `auto_fp_review` 和 `fp_review_method`
