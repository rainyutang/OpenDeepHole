...
- **修复** OpenCode/nga 子进程支持通过 `opencode.proxy_url` 或 `OPENCODE_PROXY_URL` 显式注入 `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`，避免 Agent 进程未继承交互式终端代理环境时 serve 启动阶段访问 Provider 被公司代理拦截；代理变化会触发 serve 重启
