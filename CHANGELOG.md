...
- **修复** 威胁分析超过调用时限后仍卡住并阻塞其它审计：共享 Serve 的 MCP 和事件订阅清理增加统一时限及旧代回调隔离；Task Service 增加调用与清理的外层截止保护，收尾异常也会结束结果等待。模型额度释放不再等待完成记录落盘，失败记录保留重试，本地上报队列磁盘操作移到后台线程，避免拖住 Agent 事件循环；保持原有单次调用超时、换模重试及威胁分析回退策略。需更新并重启 Agent，无需数据库迁移
- **修复** 仅运行去误报时扫描主页任务队列不显示运行和排队状态：主扫描与复核共享持续发布器，复核入队即登记计划任务，取得模型额度后转为运行状态。主扫描结束或续扫时保留当前复核的计划、排队和运行任务，按复核自身的执行版本及 Agent 会话过滤过期任务；SQLite/PostgreSQL、概览、SSE 和公开页面使用一致状态。需同步更新后端和 Agent 并重启 Agent，无需数据库迁移

- **修复** 分享页在扫描尚未创建去误报任务时误报“分享已关闭或链接无效”：前端现在精确识别扫描主概览的 `404`，去误报概览等子资源暂无数据时继续显示扫描详情并保持实时刷新，后续创建任务后同一链接可正常展示。覆盖扫描进行中、关闭自动去误报及没有问题结果的场景，保留真实令牌失效和撤销行为；只需更新前端资源，无需数据库迁移或重新生成分享链接

## 2026-09-18

- **修复** 新建扫描的项目总路径和非空扫描模块路径强制使用目标 Agent 上的绝对路径，页面提交时明确提示错误字段，后端创建接口同步拒绝相对路径，覆盖各扫描模式及多版本的每个版本。支持 Linux/macOS 路径、Windows 盘符路径和 UNC 共享路径；模块路径留空仍扫描项目总路径。需更新后端和前端，无需数据库迁移

## 2026-09-16

- **修复** OpenCode 轻量威胁分析调用校验脚本时，Python 名称、引号、空格或参数写法变化曾被完整字符串匹配误判为 `Shell command is not bound to this OpenDeepHole task`。轻量分析现在显式启用脚本识别策略：确认是直接调用任务绑定的 Python 脚本后，在执行前替换为该任务固定的完整校验命令，产物路径和参考目录不采用模型改写的值；保持原生白名单、父子 Session 绑定和执行审计一致，拒绝其它脚本、命令拼接及 shell 展开。提示词同时明确要求逐字复制命令，不增删空格、引号及参数；非 Windows 使用宿主 Python 绝对路径，避免缺少 `python` 别名时无法执行。其它任务默认仍精确匹配，宿主最终校验与失败纠正保持独立；需更新并重启 Agent 及其受管 Serve，无需数据库迁移

- **修复** OpenCode 轻量威胁分析卡住后超过超时时间仍不重试：单次调用的截止时间覆盖 Serve/MCP 准备、消息、响应处理和宿主校验，超时后的 Session 中止、请求回收及附属清理共用最多 10 秒预算；Token 补采超时保留部分统计，迟到回调不能覆盖下一次尝试。产物校验改为可取消的异步子进程，最长 60 秒且受调用剩余时间约束，恢复扫描的同步校验也增加时限。保持原有纠正、换模重试和 DeepHole 回退流程；需更新并重启 Agent，无需数据库迁移
...
- **修复** 已完成扫描重试失败候选审计时，侧栏和首页流程图的进度被旧缓存覆盖或不刷新：统一显示本轮已完成候选数 / 原有候选总数，模型池、日志及静态分析事件只更新各自字段，PostgreSQL 工作进程刷新持久化运行状态；概况轮询与 SSE 按执行轮次合并，迟到响应不能覆盖较新进度。需更新后端和前端，无需数据库迁移
- **新增** 每个扫描在列表和详情页支持生成、复制及关闭分享链接；接收者免登录查看实时状态、人工标记问题并下载单个问题报告、CSV 和 ZIP。分享使用独立令牌，后端仅开放当前扫描的读取、下载和标记操作，禁止停止、续扫、复核/验证控制及配置修改；标记实时同步，链接长期有效且可撤销，重新开启使旧链接失效。SQLite/PostgreSQL 启动自动创建分享表，需更新后端和前端，使用及接口说明见 `docs/scan-sharing.md`
- **修复** 扫描停止后续扫可能因旧任务未退出、执行轮次过早切换及重连确认顺序错误而卡住，并持续显示上一轮任务开始时间。续扫先有界确认旧执行清理，再原子领取新轮次；旧停止命令、重复续扫、过期 manifest 和迟到上报不能影响新执行。重连在 welcome 前确认活动轮次，恢复任务与 WebSocket 收包并行；清理或启动失败明确落为可重试状态，保留成功结果及历史任务，前端拒绝旧轮次覆盖新状态。需同步更新后端、前端和 Agent 并重启 Agent，无需数据库迁移
- **开发** 新增 `scripts/run_postgres_tests.py`，复用本机缓存的 PostgreSQL 和 Python 依赖，每次创建独立临时数据库并在退出后清理，测试过程不重复下载。缓存位置及用法见 `docs/testing-postgres.md`
- **修复** Agent 在首次启动 OpenCode Serve 前，将当前全部 28 个业务 Skill 及引用资源完整安装到统一 `.opencode/skills/<name>/`，任务切换只检查已安装名称，不再动态追加 `skills.paths`，避免并发审计复用旧配置后找不到 Skill。新下发规则内容变化时等待已有会话完成后发布并刷新 Serve，本地续扫使用统一当前版本；安装失败保留原目录，同名跨来源冲突明确报错。补齐 `deephole-skill-creator` 的 Agent 下载/更新包及两端哈希范围，需更新后端和 Agent 并重启 Agent，无需数据库迁移

## 2026-09-11

- **变更** 模型池只保留各模型的并发配置，总容量自动汇总已启用且配置有效的模型；移除独立全局并发闸门及内存 API 分析固定为 8 的限制，跨扫描继续共享单模型额度。旧全局并发字段兼容读取后忽略，保存、导出和下发时移除，独立 Task Agent 同步适用。配置页新增只读总容量，并在模型 ID、名称、能力、权重、模型可用并发、超时覆盖、重试次数覆盖和时间输入框上方补齐标签。新扫描及新启动阶段使用派生容量，已有阶段不动态扩充工作协程；需同步更新后端、前端和 Agent 并重启 Agent，无需数据库迁移

## 2026-09-09

- **修复** Python 3.10 部署中，扫描首页任务队列的历史分页游标因 UTC `Z` 时间解析不兼容而被拒绝，点击“加载更多”会反复回到 50 条；现在解析前统一转换时区并补齐小数秒，保留原有游标格式和排序。旧游标每次进入扫描最多自动重置一次，再次失败时保留历史及重试边界，后台刷新不再掩盖分页错误；需更新后端和前端资源，无需数据迁移
- **变更** 复核详情正文仅保留问题摘要、复核结论和阶段输出，移除独立的完整漏洞报告及“当前复核”区块；同一阶段优先显示本轮非空报告，并在阶段标题旁用标签展示报告中的 verdict，保留倾向判断、未证实和证据不足等原始含义及报告代码内容
- **修复** 多版本候选去重改为在独立工作线程执行，避免耗时比较阻塞 Agent 心跳发送和 ACK 接收而被误判离线；终端每 5 秒显示已处理候选数、已完成分组数、比较次数及耗时，并输出开始、完成、取消和失败状态。保留原有匹配规则，去重进度不计入审计数量，取消后的未完成分组不进入审计；需更新并重启 Agent
- **新增** 去误报详情中的有效最终正报可通过“查看疑似问题”打开对应问题，问题详情新增“查看去误报详情”，双向跳转自动翻页、选中并滚动定位，支持按需补读和公开分享。去误报列表改为每页 20 条并自动补齐后续数据，失败可重试；人工确认或标为误报后继续保留已有复核记录及阶段输出，复核执行资格保持原有规则，旧快照不再覆盖更新后的人工判定
- **修复** 扫描任务队列的历史任务改为先按执行结束时间倒序、再每批加载 50 条，成功、失败、超时和取消统一排序；结束时间缺失或无效时使用开始时间，无有效时间的记录排在最后，同一时刻按任务 ID 稳定分页。运行中、排队中和计划中继续置顶，刷新合并最新任务并保留已加载历史及详情；兼容公开分享、旧历史快照和升级前游标自动重载，无需历史回填

## 2026-09-08

- **修复** Windows Agent 使用 `C:\Program Files\nodejs\opencode.CMD` 等带空格路径时，OpenCode 版本查询与 Serve 启动重复拼接、转义引号，导致命令无法识别及模型列表读取失败：统一改用独立参数的 `cmd.exe /d /c call`；版本探测非零退出时保留错误日志并显示未知版本，继续尝试启动 Serve。需更新并重启 Agent
- **修复** PostgreSQL 复核概览及关联结果分页误用 SQLite `rowid` 导致 500：概览改用已有 `created_order` 处理创建时间相同的任务，保持轻量查询、汇总计数及公开访问兼容，无需重建索引
- **修复** Agent 模型池上报收到过期会话或扫描执行版本的 409 后持续重试：按原请求身份停止心跳、状态推送及退出补发，新执行自动恢复；普通失败按尝试完成时间至少间隔 2 秒重试，避免心跳到期后零等待刷请求。需同步更新并重启 Agent
- **修复** 轻量级威胁分析完成后，进入扫描页短暂显示结果数量又消失：概况与 SSE 重同步保留已加载产物，停止及续扫后的刷新沿用同一规则；结果加载统一处理完成通知、混合资源通知、重连及页面恢复可见，缺失或更新失败由轮询补拉，合并在途请求并隔离切换扫描后的迟到响应，兼容公开扫描与合法空结果
- **修复** 疑似问题页进入后自动加载全部问题、去误报结果及验证状态，筛选和列表不再依赖手动点击“加载更多”；加载中显示进度提示，失败可从未完成的分页重试，兼容公开扫描与实时更新
- **修复** 合并存储优化与审计结果导航：扫描概览使用汇总中的人工确认问题数，人工判定后刷新汇总；候选来源按正文版本恢复完整结果。候选和威胁审计详情按任务名称分页查询历史摘要，再读取选中任务的完整 Prompt，兼容旧快照和公开扫描；SSE 重连保留请求合并与审计摘要刷新
- **修复** 候选审计上报 `queued` 时，接口已允许排队状态但存储层遗漏对应校验，导致后端返回 500。现在 SQLite 及复用该逻辑的 PostgreSQL 均可持久化排队状态，并在写入前拒绝非终态携带审计结果；排队不计入已完成数量，接口响应、候选记录与 SSE 状态保持一致
- **变更** 静态分析候选详情以文件路径、行号、函数、相关变量及问题类型卡片替代“候选描述”，变量取值与实际审计 Prompt 一致；相关函数作为补充字段，原始元数据默认折叠。详情整行展示对应队列或最新历史任务保存的完整 Prompt，兼容项目级候选，未生成或未保存时明确提示
- **新增** 静态分析支持“问题：全部 / 发现问题”筛选，与现有人工判定优先、有效去误报结论次之的徽标口径一致；可组合类型和审计状态筛选，按每批最多 100 个候选点、最多两批并发覆盖所有候选页，支持加载提示、失败重试、判定更新刷新及报告返回时自动清除筛选定位

## 2026-09-07

- **新增** 威胁审计任务、静态分析候选点与问题报告支持双向定位：确认问题的审计结果可直接打开报告，报告可返回对应任务或候选点；自动调整筛选与分页并选中目标。静态候选点按人工判定优先、最近有效去误报结论次之的口径显示“发现问题”和问题简介，支持历史明确索引关联。复用页面缓存与当前页批量摘要，缺少目标时按需读取；来源缺失或歧义明确提示，公开扫描沿用现有令牌鉴权

- **变更** 问题详情页统计改为“疑似问题 / 确认问题”，确认问题统计有效去误报 TP 中人工标记为“确认正报”的数量，与首页口径一致；同步更新侧边栏和概览描述，并补齐分页问题数据以完整统计人工确认数

- **修复** 对抗式漏洞复核的最终裁决不再接受 `uncertain`：业务 Prompt、Skill 和实际执行的独立 JSON Schema 统一要求 `true_positive` 或 `false_positive`，报告正文与结构化结论保持一致；非法裁决进入既有输出纠正流程，避免任务显示成功但复核未形成有效结果。正反方中间阶段保留原规则，历史未完成项可通过“启动复核”补跑

- **新增** 基于攻击威胁的漏洞挖掘引擎任务详情展示审计结果、问题简介、代码位置及最新判定；左侧仅在人工确认或有效去误报确认后显示“发现问题”，误报和未确认结果不添加标签。新旧扫描通过同一扫描内的任务来源、结果索引及唯一历史节点/模式/路径关联读取结果，支持多结果和稀疏索引；记录不足时明确提示，当前任务页按批查询并随结果及复核更新刷新，公开扫描沿用访问令牌校验

- **优化** 扫描历史任务以不可变版本保存正文，终态接收记录引用同一版本；心跳只更新当前队列与计数，Token/模型快照仅写入变化行。候选与漏洞共享业务范围内的审计正文，去误报结果固定引用当次阶段，保留旧报告、Session、人工结论和继续扫描信息，不增加应用层压缩
- **优化** 扫描列表与概览读取显式列、标量计数及事务内维护的汇总，统计面板改为权限/产品范围内的 SQL 聚合与检查器扫描分页；任务、复核及公共详情默认分页 50 条，正文按需读取。SSE 以资源变化通知为主，提交后广播，溢出和重连重新同步
- **新增** Agent 能力协商支持验证日志分段追加、完整输出和同字段引用；持久 outbox 保存序号及未确认分段，重启与重复送达均可恢复，旧 Agent 保留完整快照兼容路径
- **新增** 有租约和批量上限的运行数据清理，按终态保留 Agent 命令、失效会话、RPC、manifest 与 SSE 传输缓存；不为扫描业务历史或未确认 outbox 设置 TTL。扫描删除使用持久标记、活跃任务检查和可续跑的分批删除，保留独立反馈及项目目录
- **修复** 旧启动迁移及日志写入会删除超过 200 条的扫描事件；现在仅限制旧接口返回的日志窗口，历史数据库事件保留并可分页读取，升级时也不再删除已经保存的旧 Task Agent 事件
- **新增** 生产电脑本地执行的历史迁移工具，提供只读检查、兼容扩展、分批续跑、正文/引用核对、观察 7 天后的显式重复正文清理与停写旧字段重建。PostgreSQL 索引并发创建、外键分阶段验证；异常或超大原始记录停止并保留源数据。操作步骤见 `docs/database-storage-migration.md`，本机 1 万扫描/10 万任务的查询对比见 `docs/database-storage-benchmark.md`

## 2026-09-04

- **修复** PostgreSQL 旧库升级时，启动引导会在为 `scan_candidates` 补充候选审计列之前先创建依赖 `audit_state` 的索引，导致服务以 `UndefinedColumn` 退出。现在所有从 SQLite Schema 派生的索引统一延后到兼容补列完成后创建，新库初始化与旧库幂等升级使用同一顺序
- **修复** 多版本审计续扫此前只复用威胁分析和模型审计任务，组合引擎仍会从第一个版本重新执行静态规则扫描。现在每个版本静态分析成功后立即按版本路径、checker 选择和去重配置原子保存完整候选检查点；续扫会直接恢复合法的完整或空结果，并跳过该版本在组合引擎内的索引准备与静态扫描，只重跑未完成、缺失、损坏或身份不匹配的版本，基准版本的扫描级索引缓存检查及原有候选归并、模型任务复用和上报链路保持不变
- **变更** Task Agent 在完整 Agent、standalone、模型列表和任务启动共用的最终 `opencode.json` 中全局允许 `read`、`list`、`glob`、`grep` 与 `external_directory`，使 OpenCode 可读取运行账户有权访问的全部本地路径，跨版本审计不再依赖 Session 是否正确携带每个真实版本目录。写入边界保持独立收敛：`edit` 继续默认拒绝，并保留 `scans`、`fp_reviews`、`vulnerability_validation`、`skill_create`、当前 `work_dir` 及动态写白名单的原有允许规则，`bash` 仍默认拒绝；旧受管配置中的全局外部目录拒绝会在初始化时自动刷新
- **修复** 扫描续跑时，为了给依赖引擎恢复原生威胁分析对象，后端曾把“需要已有结果”误判为“需要重新分析”，先将已经成功的威胁分析状态重置为待执行，Agent 又直接启动新任务。现在续扫优先从服务端取回、校验并安全恢复同一扫描的三份威胁分析产物，命中时保持成功生命周期且不创建计划任务、模型 Session 或重复上传；结果缺失、损坏或扫描范围不匹配时才用全新 Session 重跑。失败的 Codex/OpenCode 轻量威胁分析不再续写旧 Session/线程，也不会继承残留产物
- **新增** `threat_pattern_audit` 续扫按稳定的“高风险模块 + 攻击模式”任务 ID 复用已有合法结果，包含确认漏洞和 `[]` 无漏洞结论；缓存结果重新进入既有上报与去重链路，只有缺失或 Schema 校验失败的组合才创建新 Session。多版本模式同步为基线威胁分析、基线攻击模式审计、跨版本静态候选组、逐版本差异威胁审计及后续同根因匹配保存结构化成功检查点，续扫命中时不进入模型队列；升级前已完整的基线威胁分析与攻击模式结果可兼容导入，未保存检查点的其它多版本模型任务首次续扫会重跑一次
- **优化** 多版本静态候选审计不再把候选和版本对象直接作为 JSON 交给通用 Skill；现在按 `vuln_type` 复用 `static_candidate` 的 checker 目录与 `checker.yaml.skill_name` 选择对应问题专用 Skill，以文件、行号、函数、相关变量和问题类型构造精简审计问题，并追加多版本等价实现定位、跨调用路径校验差异、修复差异、接口契约、配置和生命周期变化等审计技巧。静态候选位置只列出实际命中的版本，不再为未命中版本输出占位说明；输出继续逐版本给出存在性、真实位置和理由
- **优化** 对照版本差异威胁审计改为先系统梳理与基准版本之间的功能和代码差异，再围绕全部新增或变化部分查找所有可能安全问题并逐项验证；基准威胁分析线索只作优先级参考，不再限制审计类型和范围，差异结果数组也移除最多 10 项的限制
- **优化** 多版本已确认漏洞的跨版本同根因匹配提示词不再直接嵌入完整漏洞与版本 JSON，改为明确展示发现版本、漏洞位置、详细证据和逐个待检查版本，并用简短规则说明同一漏洞判断标准及 `affected_versions` 输出要求
- **变更** 多版本模式的基准版本威胁审计改为直接复用现有 `threat_pattern_audit`：列表首项完成威胁分析后，其原始产物由该单版本引擎消费，不再使用 `/multi-version-vulnerability-audit` 或旧 `threat_audit` 任务拆分结果自行构造基准审计任务。基准版本已确认问题仍会逐版本匹配同一根因并保存版本标签与位置，对照版本相对基准的新增或变化威胁审计保持不变，`threat_pattern_audit` 本身代码未修改
- **修复** 正式漏洞即时创建去误报任务后，扫描结束补调度或续扫可能把服务端 FP review 的 execution revision 提升，但延迟到达的漏洞回调又把 Agent Reporter 降回 revision 0，导致 `prove_bug`、`prove_fp`、`final_judge`、最终裁决和完成状态被后端按 stale 409 丢弃，页面长期显示“审计中”。后端现在为同一 Agent process 原子复用活动复核 revision，仅在手动重跑、进程切换或终态重开时递增；即时回调显式返回并校验 session/revision，Agent 在队列去重前先单调同步执行身份。新旧 Agent/后端混合部署时自动回退到扫描结束调度，已有卡住任务在 Agent 重连后只补跑未形成结果的项目，已保存最终结果但缺少 finish 的任务直接收敛为完成；多版本创建页保持不变
- **修复** 扫描详情流程图中的“DeepHole基于攻击模式的漏洞挖掘引擎”不再在运行后只显示引擎 ID；引擎确定高风险模块与攻击模式组合后，会通过既有运行状态、持久化和 SSE 链路实时上报已处理任务数与总数，框图及左侧流程导航显示 `x/y 已审计`。成功、无漏洞和失败的已结束任务都会推进进度，被取消的未完成任务不会计入；旧扫描缺少计数时明确显示无历史进度数据
- **变更** Agent 的 `code_index.db` 从项目总目录收敛到实际 `code_scan_path`，数据库只索引并缓存该扫描路径，静态候选与多版本引擎使用返回的扫描路径索引；仅当本轮实际运行 `static_candidate` 或 `multi_version` 时才构建，纯威胁分析及其它非静态引擎扫描会明确跳过索引阶段。扫描子目录时不会再读取、迁移或删除项目根目录遗留索引，多版本的每个版本也分别写入自己的扫描路径；扫描详情同步显示索引已跳过，独立 CodeGraph MCP 保持不变
- **修复** 去误报公共 Prompt 构造器在 JSON Schema 后统一追加“请使用中文输出”，恢复 `adversarial` 和 `fp_check` 每个阶段的中文输出约束；Slash Skill、报告上下文、Schema 与重试纠错提示保持不变
- **修复** 所有漏洞挖掘引擎共用的正式漏洞上报在启用持久化 outbox 后，如果同一扫描已有 OpenCode 任务终态或另一条漏洞正在发送，曾会因当前请求暂时无法取得发送权而丢失后端响应，导致已确认问题虽然成功入库却不创建即时去误报/漏洞验证任务；Agent 现在为当前进程中的漏洞报告保留按 outbox 行与代次隔离的一次性送达回调，在线时等待有序发送完成后再返回，短暂离线时不阻塞扫描并在后台补传成功后继续派发。各内置引擎仍只使用统一 `report_vulnerabilities()` 回调，多版本创建页及其配置交互保持不变

## 2026-09-03

- **变更** 快速和标准扫描的固定漏洞挖掘组合从 `static_candidate + threat_audit` 调整为 `static_candidate + threat_pattern_audit`，改为按“高风险模块 + 攻击模式”创建审计任务；威胁分析失败后续扫会补跑所有受其依赖关系阻塞的挖掘引擎，原 `threat_audit` 继续供自定义模式和历史扫描使用，多版本模式保持不变
...
- **修复** Agent 通过 v1 单条或 v2 批量接口上报扫描事件时，若扫描已删除、请求命中未包含该扫描的存储实例，或事件在扫描结束后迟到，不再先向 `events` 写入并触发 SQLite/PostgreSQL 外键异常和 ASGI 500；后端现在在接口和存储事务两层确认父扫描仍存在，缺失时返回带 endpoint 与存储指纹日志的结构化 `scan_not_found` 404。已存在终态扫描的迟到事件仍会持久化但不会恢复运行状态，Task/Session/Tool 内部输出过滤、批量计数、SSE 和每个扫描最多保留 200 条事件的行为保持不变
...
- **修复** Agent 重连改为服务端权威的执行会话/revision 对账：同一进程仍在执行的扫描、去误报和验证只迁移连接，进程重启后缺失的非终态或“Agent 断开连接”任务会原子认领并从持久化检查点自动恢复，客户端 outbox 中已有待发终态时先等待补传，旧执行的迟到报告不会覆盖新执行。运行时自动更新发现本地仍有任务时不再替换进程，Agent 启动日志会区分运行时更新重启与初次启动/外部守护进程拉起；恢复未新增客户端数据库表或记录，只读取现有未确认 outbox 行，已被服务端判定过期的行立即删除，避免额外持久存储增长
- **修复** PostgreSQL 重启后，后端不再把执行、取数、提交或回滚失败的连接永久绑定在线程中并反复复用：连接池借出前会先验活，异常清理始终解除线程绑定并将失效连接交回连接池替换；跨 Worker 的 Agent 命令和 SSE 补读循环遇到数据库中断时也会持续退避重连。数据库不可用期间请求仍可失败或超时，恢复后无需重启后端即可由下一次请求继续读写，结果未知的写事务不会被自动重放
- **修复** Agent 在后端离线期间不再只让扫描继续执行却丢失候选审计、漏洞、威胁审计、去误报及验证终态：权威结果现在先进入仅保存未确认项的本地 SQLite outbox，同一扫描严格按序补传，收到后端 2xx 后删除并增量回收空间；后端以稳定业务身份幂等接收，响应丢失后的网络重放不会形成重复结果，并可在到达顺序变化时恢复候选/威胁任务与漏洞索引关联。该修复只保护升级后新产生的数据，不补造历史缺失结果
- **优化** OpenCode 模型池快照继续累计上报 token、完成数和当前队列等标量，但不再反复携带随 Session 数量增长的完整完成历史；后端 capability 协商后，逻辑任务的成功/失败、Session ID、错误分类及 Session 尝试轨迹改为按 `task_id + revision` 单点持久化上报并通过增量 SSE 合并，重复请求只确认不重复落库。旧后端仍使用原累计历史格式，扫描完成报文额外兜底最新 token/计数快照
...
- **修复** Codex CLI、模型、托管配置不可用，或 Codex 威胁分析执行失败、结果产物无效时，扫描外层编排会归档已有失败产物并仅以 clean 模式回退一次 DeepHole 威胁分析；用户取消不触发回退，DeepHole 回退失败会保留两段错误原因。Codex SDK 和两种威胁分析方法实现均未修改

## 2026-08-29

- **修复** 首页扫描历史的“疑似问题”列改为展示去误报后的有效最终 TP 总数，不再扣除已提交人工反馈的问题；未标记、待分析、确认正报和实为误报均计入该总数，“确认问题”列仍仅统计人工确认正报
- **修复** 静态候选审计统一以扫描内 `ScanCandidate.idx` 作为唯一身份：候选记录直接持有唯一 `audit_state`、唯一 `audit_result` 及可选漏洞投影索引，开始、终态覆盖、断点恢复和历史迁移均不再按文件/行号/函数关联；项目级候选同步收敛为单对象输出，多个真实问题只保留风险最高且证据最完整的一条。命中同模式过滤的候选显示审计成功，并固定标记“候选点去重：同模式代表点已被 AI 审计为非问题，本候选未再次调用模型。”；旧漏洞结果只在 `audit_index` 能合法对应候选 `idx` 时回填且不会按位置猜测
- **修复** 扫描详情概览新增候选审计和威胁审计的完整分状态聚合计数；候选“已审计”数改由候选终态聚合，不再因静态引擎完成而强制等于总数；流程图、侧栏和任务面板首屏不再用候选或威胁任务首批分页（100/200 条）的数组长度作为总数，分页仍按需加载具体明细
...
- **修复** 多个漏洞上报批次并发对账后，后端保留的稳定漏洞索引可能出现合法空洞，扫描详情前端此前按绝对索引构造并展开稀疏数组，使空洞变成 `undefined` 并在统计 `variant_of` 时触发整页 Error Boundary：漏洞集合现在使用携带真实 `vuln_index` 的稠密列表，初始加载、公开分享、分页和 SSE 统一按稳定索引合并，FP 复核、验证、人工反馈及静态分析映射也不再依赖数组位置；后端索引、接口报文和用户操作流程保持不变

## 2026-08-25

- **修复** 客户端从 OpenCode 兼容 Serve 导入模型时，复用中的 `opencode`/`nga` 进程此前会跳过健康检查直接访问 Provider，且 Serve 启动 INFO 日志因 logger 命名空间未接入 Agent 控制台而不可见：模型导入现在强制刷新并在 `/provider` 前要求 `GET /global/health` 返回 `200` 与 `healthy: true`，空闲异常 Serve 只安全重启一次，活动 Session 不被强制中断；Agent 终端按时间输出请求编号、executable 配置与解析路径、版本、完整启动命令、端口、健康检查、Provider 耗时和脱敏启动输出。WebSocket 心跳与模型结果发送统一串行化并补充心跳/看门狗失败原因，避免并发发送异常静默演变为 120 秒后断线
- **修复** 扫描已取消或因 Agent 断连而中断后，持久化模型池仍残留排队中、计划中或运行中任务，且 PostgreSQL 多 Worker 的本地缓存可能让续扫误报 `Scan is already running`：扫描进入完成、失败或取消终态时会原子清空模型池瞬时任务并保留完成历史与 token 统计，历史终态记录读取时也应用同一清理，迟到的活动快照不会重新写回排队状态；续扫以数据库状态为准并通过条件更新原子抢占，避免陈旧 Worker 拒绝有效续扫或并发重复下发。续扫命令仍在 Agent 真正恢复任务前执行当前 runtime 更新校验，版本不一致时先安装并重启后自动接续，同版本不重复安装

## 2026-08-24

- **优化** OpenCode 威胁分析任务优先级由 `75` 调整为与漏洞挖掘相同的 `50`，两类任务按现有 FIFO 顺序共享模型容量，使静态候选点审计可在威胁分析持续产生任务时穿插执行；已运行任务仍不抢占，漏洞验证与去误报优先级保持不变
- **优化** 基于代码风险点的候选审计队列固定优先处理“异常分支内存泄漏”，其它规则按本次待审计候选数量由少到多排列，同数量规则及规则内候选保持稳定顺序；新扫描、续扫和定向重试共用该规则且保留现有并发 worker。疑似问题列表外观和交互不变，仅在内部稳定排序中将候选点审计结果置于攻击威胁审计结果之前
- **优化** 新建扫描页的三种扫描模式卡片移除介绍并只保留模式名称；产品改为无候选下拉的直接文本输入，去掉括号提示，并在网页创建时与项目总路径一同作为必填项，存量扫描及服务端和外部集成接口保持兼容
- **优化** 扫描报告 CSV 新增“人工反馈”列，按问题当前人工标记显示“确认正报”“实为误报”“待分析”或“未标记”；原有去误报最终 TP 导出范围、去重顺序及 ZIP 报告对应关系保持不变
- **优化** 新建扫描页将知识库与漏洞验证移到代码图谱下方，两个卡片只显示“实现中...”并将启用开关置灰为不可操作；原有后端配置、历史扫描数据与运行逻辑保持兼容
- **优化** 新建扫描页移除扫描基本信息与扫描模式的冗余说明，将四项基本信息改为纵向排列并把“代码扫描路径”更名为“扫描模块路径”；代码图谱配置收敛为默认关闭的右侧开关，启用后固定使用项目总路径下的本地 CodeGraph，不再提供远端连接、连接检测及额外配置
- **优化** 首页扫描历史的问题统计与问题页统一为去误报后的有效最终 TP 口径：“疑似问题”仅统计人工未标记或标记为待分析的结果，确认正报和实为误报均从疑似数中扣除；“确认漏洞”同步更名为“确认问题”，仅统计最终 TP 中被人工标记为确认正报的数量
- **优化** 扫描详情侧边栏将内置方法名“DeepHole威胁分析”简化为“威胁分析”，并隐藏尚未产出结果时的“等待生成攻击树”占位说明；顶部流程图、结果页方法名称、运行状态及有效摘要或错误信息保持不变
- **优化** 扫描详情侧边栏的“疑似问题”摘要改为展示问题总数与已验证数；疑似问题列表明确区分人工未标记、待分析、确认正报和实为误报四种状态，并新增仅作用于该页面的人工状态筛选，提交、修改或取消标记后列表与筛选计数即时更新
- **修复** `multi_ptr_leak2` 静态分析器遍历 tree-sitter AST 时改用保持原前序顺序的显式栈并设置 4096 层深度上限，避免源码语法树嵌套超过 Python 递归上限后触发 `RecursionError`，超过安全上限时改为报告明确错误而非无界遍历；结构体、函数、符号及释放点识别行为保持不变
- **优化** 候选点审计、威胁审计以及 `adversarial` / `fp_check` 两种漏洞复核方法的首轮业务 Prompt 在现有上下文、输出要求和 JSON Schema 之后明确要求使用中文输出；Task Agent 原样透传、同 Session 续写、JSON 格式化与纠错提示词以及其它模型流程保持不变
- **优化** 扫描详情侧边栏“执行流程”同步顶部流程图的主要阶段，依次展示程序分析、多引擎问题识别、问题复核和验证利用，“疑似问题”仍位于“扫描概览”的“首页”下方；流程图与侧栏将原“漏洞验证”展示为“验证利用”，并仅把“DeepHole基于代码风险点的漏洞挖掘引擎”展示为“DeepHole基于代码风险点的问题识别引擎”，“DeepHole基于攻击威胁的漏洞挖掘引擎”及底层执行标识保持不变
- **优化** 新建扫描模式调整为默认可选的标准模式、不可点击且显示“实现中...”的智能模式占位以及自定义模式：标准模式沿用现有规则和固定流程，自定义模式继续按需配置威胁分析、漏洞挖掘引擎及去误报方式；快速模式保留后端、历史扫描、续扫和高级配置兼容，但不再出现在创建页面
- **优化** 扫描详情“导出报告”菜单收敛为仅显示 ZIP 和 CSV；两种格式统一只导出去误报已形成有效 TP 最终结论的当前问题，并共享相同的去重、顺序和 Markdown 文件名。CSV 固定为“文件、行号、函数、问题类型、问题描述、ZIP中的问题报告”六个中文列，最后一列与压缩包内对应报告一一匹配
- **优化** 扫描详情侧边栏及首页指标将“发现的问题”统一更名为“疑似问题”，列表仅展示 AI 去误报有效最终裁定为问题的结果；主问题页筛选收敛为类型、引擎和验证，列表与详情移除严重级别、引擎、匹配、正反方等过程标签，只保留问题位置、人工标记状态、漏洞报告、输出来源及最终裁定；静态规则问题类型优先使用现有规则清单中的中文名称，原始规则 ID 继续用于筛选并保留在悬浮提示中
- **修复** Windows Agent 启动脚本不再仅通过 `where.exe` 判断 Python 命令是否存在，改为依次执行 `python3 --version` 和 `python --version` 并只选用成功返回的解释器，避免 Microsoft Store 执行别名被误认为可用 Python 而跳转到下载页面
- **优化** 扫描详情流程图改为按内容宽度紧凑居中展示：威胁分析四个纵向入口分别显示价值资产、高风险模块、内部节点和攻击树数量；渗透执行依次展示程序分析、多引擎问题识别、问题复核、漏洞验证和等高的“疑似问题”入口，不再拉伸占满页面；配置关闭的阶段明确显示“未启用”，并与无目标时的“已跳过”和具体引擎的“未选择”区分
