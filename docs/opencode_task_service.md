...
只有传入 `output_schema` 时才解析结构化结果。服务不使用 OpenCode 原生 `format=json_schema`，而是解析普通 assistant 文本。

若输出不是符合 Schema 的 JSON，服务会在原 Session 自动追加纠正提示，重复 Schema 并要求只输出合法 JSON。最多追加 `invalid_json_retry_count` 次；这些纠正消息复用同一 Session、同一模型 Lease，不重新排队。
