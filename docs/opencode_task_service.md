...
Agent 启动时只注册一次 OpenDeepHole 的配置、workspace 与 MCP/SKILL 适配，不创建 Serve 进程。首次 `run_opencode_task()` 会惰性创建共享任务服务和 Serve 管理单例；组件在真正发送 prompt 前检查 Serve，按现有规则启动、复用兼容进程或恢复异常进程。后续所有阶段复用同一单例和 Session 管理逻辑，不存在额外的组件 CLI。
...
`agent/opencode/` 内不导入 OpenDeepHole 的 `agent`、`backend`、`mcp_server` 或 `code_parser` 模块；单独复制该目录时，只需提供 `httpx` 并注册 `OpenCodeHostBindings` 即可作为独立 Python 组件使用。OpenDeepHole 业务阶段只能从 `agent.opencode` 导入公共类型和函数，不应直接依赖 `task_service.py` 中的内部任务记录、句柄或 Session 管理方法。
