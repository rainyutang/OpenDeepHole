...
8. 单个引擎失败会被隔离。至少一个已选引擎成功时，扫描可以带警告完成；全部引擎失败时
   扫描失败。仅运行威胁分析时，以威胁分析结果决定扫描状态。
9. 取消信号会向过程和引擎传播；组件必须尽快停止新工作并清理自己启动的子任务或子进程。
10. 续扫按持久化阶段生命周期定向执行。依赖引擎需要结果时，Agent 先校验并恢复服务端保存的
    成功威胁分析产物，不重置成功生命周期，也不创建新 Session；结果缺失或非法时才新建任务。
    轻量级方法可以在 `is_resume=True` 时识别本地完整成功产物，但不得续接失败 Session。
    `threat_pattern_audit` 和多版本引擎同样只复用通过结构校验的成功 Session 结果，失败、取消或
    超时的单项任务使用新 Session 重跑，不做 Session 内续接。
...
未知 key 应立即报错；返回值应可 JSON 序列化。所有过程都会在 `kwargs["scan_mode"]` 中收到
`quick`、`standard` 或 `custom`，相同值也写入 Task Agent 上下文的任务元数据。历史
`full` 和 `threat_analysis_only` 扫描在组件边界统一映射为 `custom`。威胁分析的外层入口仍由相邻的
...
| 分类 | 参数 | 说明 |
| --- | --- | --- |
| 身份 | `engine_id`、`engine_label`、`scan_id`、`scan_mode` | 本次扫描固化的引擎身份、扫描标识和模式 |
...
| 能力 | `config`、`code_graph_mcp`、`knowledge_base_mcp`、`codex_command`、`codex_models` | Agent 只读配置、本次扫描私有 MCP 配置，以及仅向声明依赖 Codex 的引擎提供的裸 argv 前缀和无密钥默认模型元数据 |
...
- `requires_codex` 可以省略，缺省为 `false`；设为 YAML 布尔值 `true` 时，Agent 启动会提前
  ...
  取得可直接追加参数的无 shell argv 前缀，并从 `kwargs["codex_models"]` 取得当前有效默认模型。
  列表最多一项，包含 `id`、`provider_id`、`model_id` 和与 `codex_command` 相同的裸 `command`，
  不包含 URL 或凭据；没有有效默认模型时列表为空。
- 未知字段、缺失字段或非法目录只会隔离当前引擎，不影响其它有效引擎发现。

Codex 配置应用时先检查 `$CODEX_HOME/config.toml` 的用户顶层 `model`；存在时直接使用且不探测、
不覆盖。不存在时，框架才按平台下发的有序显式模型快照向 `/v1/responses` 发送最小非流式请求，
并把第一个可用模型写入 `config.toml` 文件开头的托管默认区，同时生成由
`model_catalog_json` 引用的内容寻址模型目录。OpenCode 的正整数 `models.<id>.limit.context` 会作为
Codex 上下文窗口；无法读取时使用 `128000`，自动压缩阈值按 `128000` 对应 `100000` 等比例计算。
用户已有的模型目录指针、上下文窗口或自动压缩阈值均不覆盖。选中地址的主机同时写入
`$CODEX_HOME/.env` 的 OpenDeepHole 托管区，并分别追加到文件中的 `NO_PROXY` 和 `no_proxy`；
框架不会向当前进程或子进程注入这两个变量。旧版托管 profile 和不再引用且所有权标记有效的旧模型
目录会被清理，用户配置与非托管文件保持原样。扫描只读取已经固定的默认配置，不再逐扫描探测或
切换模型。
