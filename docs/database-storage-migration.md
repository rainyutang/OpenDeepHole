...
任务队列中运行中、排队中、计划中的任务继续置顶，历史任务按执行结束时间倒序从最新的 50 条开始加载，成功、失败、超时和取消不分优先级。结束时间缺失或无效时取开始时间，没有有效时间的任务排在最后；延迟补报或迁移入库不会改变执行时间排序。表格仍每页显示 12 条。登录及公开分享的 `/tasks` 接口共用此规则，并继续支持 `task_name` 精确筛选。

历史摘要额外返回规范化 UTC 时间 `sort_time`（保留微秒；无有效时间为 `-`），游标包含该时间和任务 ID，按两者倒序继续向历史翻页。调用方应原样传回 `next_cursor`，不自行拼接游标；升级前的单任务 ID 游标会返回 `400 Invalid cursor`，新版任务队列会清除旧分页状态并从最新首批重新加载一次。任务正文仍按需读取；此调整复用现有时间字段，兼容旧快照，无需新表、索引或历史回填。PostgreSQL 时间解析使用仓库生产基线 PostgreSQL 16 的有效性检查，避免无效历史时间导致分页查询失败。
