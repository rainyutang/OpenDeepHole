...
扫描历史增加后的容量、读写放大、清理和归档设计，见 [数据库容量与性能排查及优化方案](database_optimization_plan.md)，其中提供隔离复现工具和生产 PostgreSQL 只读诊断 SQL。
...
- 候选审计终态、漏洞、威胁分析、威胁审计终态、去误报结果、验证终态和扫描完成等权威结果先写入 Agent 本地 SQLite outbox，再尝试 HTTP 上报。网络失败不会反向中断仍在运行的审计；同一扫描按入队顺序补传，只有后端返回 2xx 才删除本地记录。当前 Agent 进程会为正式漏洞保留一次性送达回调：在线时在有序发送拿到响应后继续派发去误报/验证，短暂离线时先让扫描继续并在补传成功后派发。新版回调显式声明支持 FP execution revision，后端验证漏洞来源 session 与扫描当前 session 后原子分配或复用 revision；扫描结束补调度沿用相同 revision，Agent 对延迟回调只允许单调升级。任一端为旧版本时跳过即时派发并由扫描结束命令兜底。后端按候选索引、漏洞身份、任务 ID/revision、复核 ID/漏洞索引等稳定业务键幂等落库，因此响应丢失后的重放不会产生重复业务结果。
