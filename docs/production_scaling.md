...
- Agent 协议 v2 将候选点和事件分批上报；候选审计以 `ScanCandidate.idx` 回写唯一状态/结果并据此续扫，旧版按位置组成的 processed key 端点仅保留滚动升级兼容。结束报文只携带终态计数；被下游接受的漏洞投影仍逐条实时上报。
- 候选审计终态、漏洞、威胁分析、威胁审计终态、去误报结果、验证终态和扫描完成等权威结果先写入 Agent 本地 SQLite outbox，再尝试 HTTP 上报。网络失败不会反向中断仍在运行的审计；同一扫描按入队顺序补传，只有后端返回 2xx 才删除本地记录。后端按候选索引、漏洞身份、任务 ID/revision、复核 ID/漏洞索引等稳定业务键幂等落库，因此响应丢失后的重放不会产生重复业务结果。
...
- Agent 重连时会把当前进程的活动扫描、去误报、验证以及现有 outbox 待发终态身份与服务端持久状态对账。服务端在原扫描、复核和验证行上保存固定大小的 Agent process session 与单调 execution revision：活动任务只换连接，进程重启后缺失的任务原子增加 revision 并从已有检查点恢复；“Agent 断开连接”属于基础设施中断，可以在宽限期后继续恢复，用户取消不恢复。待发终态先补传，旧 revision 的 409 报告直接删除。客户端不新增恢复表、恢复行或完成历史，因此该机制不会扩大 outbox 的持久存储边界。
...
- 候选审计和威胁审计的“排队中”只来自真实模型租约等待队列，“运行中”只在取得租约后产生；扫描级快照在首次租约前也包含已配置模型的零值容量行。模型池 POST 会检查非 2xx，失败时每 2 秒以内重试最新快照；若服务端以 413 拒绝，Agent 去除 Prompt 和非身份明细、标记 `details_truncated` 后重发。
