...
**扫描模式与流程**：新建扫描依次展示标准、智能、自定义三种模式，默认选择标准模式；智能模式当前仅显示“实现中...”且不可选择。标准模式固定运行“DeepHole基于代码风险点的漏洞挖掘引擎”与“DeepHole基于攻击模式的漏洞挖掘引擎”，固定启用威胁分析、基于攻击模式的审计和对抗式复核去误报，并只默认排除测试规则 `skill_only_project_audit`。自定义模式可按需配置威胁分析、漏洞挖掘引擎及去误报方式。快速模式不再出现在新建扫描页面，但服务端、历史记录、续扫和客户端高级配置继续保留兼容，并使用与标准模式相同的固定引擎组合。扫描详情主流程保持“威胁分析 → 漏洞挖掘 → 漏洞验证”，去误报从漏洞挖掘之后并列分出；选择静态分析引擎时，代码图谱完成后威胁分析与无需其产物的挖掘工作可并行，未选择时直接跳过代码图谱，攻击模式引擎仍等待威胁分析成功后再启动。
