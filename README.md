...
**扫描模式与流程**：新建扫描提供快速、标准、自定义三种模式，默认选择快速模式。快速和标准模式固定运行“DeepHole基于代码风险点的漏洞挖掘引擎”与“DeepHole基于攻击威胁的漏洞挖掘引擎”，固定启用威胁分析、威胁审计和自动去误报；快速模式默认排除 `sensitive_clear` 与测试规则 `skill_only_project_audit`，标准模式只默认排除测试规则。自定义模式保留原有的威胁分析开关及方法、漏洞挖掘引擎选择、自动去误报开关及方法。扫描详情主流程保持“威胁分析 → 漏洞挖掘 → 漏洞验证”，去误报从漏洞挖掘之后并列分出；代码图谱完成后，威胁分析与无需其产物的挖掘工作仍可并行，攻击威胁引擎等待威胁分析成功后再启动。
...
**AI 去误报（扫描级开关与目录化方法）**：快速和标准模式固定启用自动去误报并使用目录清单声明的默认方法；自定义模式可选择是否自动去误报，并从 `deephole_client/fp_review/methods/` 自动发现的方法目录中选择一种。每个方法都通过统一的 `async def run(**kwargs)` 入口一次处理一个漏洞，输入包含本次扫描代码路径、漏洞索引和单个漏洞对象；平台负责扫描级排队、方法并发、取消、补跑和持久化。扫描详情只在漏洞复核页提供统一操作入口：存在未形成有效 TP/FP 结论的项目时显示“启动复核”并只补跑这些项目，全部项目均已有有效结论后显示“重新复核”并在点击时全量重跑；“续扫”同样会恢复所有未形成有效结论的项目，包括从未启动过复核的扫描，但不会重跑已经全部完成的复核。内置 `adversarial` 保留 `history_match`、`prove-bug`、`prove-fp`、`final-judge` 流程，内置 `fp_check` 执行主张重述、标准/深度验证与六道门，最多并发四项。方法名称、默认项、最大并发、阶段和说明文档均来自各自的 `method.yaml`，扫描创建后会固化所选方法快照；方法不得生成跨漏洞批次汇总或修改其它漏洞结论。
**漏洞报告导出**：对每一个 AI 去误报形成有效 TP 最终结论的扫描项可单独导出 Markdown 报告，包含所选去误报方法的阶段证据；扫描详情页顶部「导出报告」提供 ZIP 和 CSV，两种格式导出同一批当前问题。CSV 固定包含“文件、行号、函数、问题类型、问题描述、ZIP中的问题报告”六列，最后一列指向 ZIP 内对应的 Markdown 文件。对应端点为 `GET /api/scan/{id}/vulnerability/{idx}/report`、`GET /api/scan/{id}/report` 与 `GET /api/scan/{id}/report.zip`。
...
下载包会自动填入 `server_url` 和 `owner_token`。首次启动并连接后，在 Web UI 的 **「客户端配置」** 页面按机器名与 IP 选择客户端。页面只有「基础配置」和「高级配置」两个子页：基础配置包含基础参数与模型配置，高级配置包含威胁分析、漏洞挖掘、去误报和漏洞验证。漏洞挖掘区按快速、标准、自定义三个模式分别选择基于代码风险点引擎的 Checker；保存后从下一次新建扫描生效，未列入禁用项的后续新增 Checker 会在各模式中自动启用。漏洞验证区保留所有方法共用的漏洞类型、并发、重试和模型策略。新注册客户端的模型列表固定为空；必须手动添加并启用至少一个明确的 `provider/model` 后才能创建或续建扫描。服务端会持久化配置并推送给在线客户端，已有稳定客户端重连时保留原配置，离线编辑会在重连后生效。

所有登录用户都可以打开 **「结果看板」**。管理员查看全部扫描，普通用户只汇总自己创建的扫描；看板顶部的 Token 总计覆盖当前权限范围内的全部扫描，按 Agent 分组且不受产品筛选影响。单次扫描的模型和任务 Token 明细仍保留在扫描详情中。

**「新建扫描」** 页面先集中填写扫描名称、项目总路径、代码扫描路径和产品，再选择快速、标准或自定义模式，默认快速模式。只有自定义模式显示威胁分析、漏洞挖掘引擎与自动去误报的原有选项。页面不直接选择 Checker；创建时由服务端读取所选模式对应的客户端规则配置并固化快照，续扫继续使用原快照。自定义模式的引擎清单直接来自当前代码仓 `deephole_client/vulnerability_mining/engines/`。
