...
[用户本地]
  ...
  ├── 等待服务器通过 WS 推送扫描任务
  ├── 收到任务后：按需构建代码图谱 → 按选择运行分析与漏洞挖掘 → 验证/去误报
...
**扫描模式与流程**：新建扫描依次展示标准、智能、自定义和多版本测试四种模式，默认选择标准模式；智能模式当前仅显示“实现中...”且不可选择。标准模式固定运行“DeepHole基于代码风险点的漏洞挖掘引擎”与“DeepHole基于攻击模式的漏洞挖掘引擎”，固定启用威胁分析、基于攻击模式的审计和对抗式复核去误报，并只默认排除测试规则 `skill_only_project_audit`。自定义模式可按需配置威胁分析、漏洞挖掘引擎及去误报方式。多版本模式的静态候选审计按问题类型复用 `static_candidate` 对应 checker 的专用 Skill，并追加跨版本校验、防护和调用路径差异审计技巧；列表首项完成威胁分析后，基准版本威胁审计复用 `threat_pattern_audit`。`/multi-version-vulnerability-audit` 仅继续用于已确认问题的跨版本匹配，以及对照版本相对基准的新增或变化威胁审计。快速模式不再出现在新建扫描页面，但服务端、历史记录、续扫和客户端高级配置继续保留兼容，并使用与标准模式相同的固定引擎组合。扫描详情主流程保持“威胁分析 → 漏洞挖掘 → 漏洞验证”，去误报从漏洞挖掘之后并列分出；代码图谱完成后，威胁分析与无需其产物的挖掘工作仍可并行，攻击模式引擎等待威胁分析成功后再启动。
**失败续扫**：服务端根据独立威胁分析和各漏洞挖掘引擎的持久化生命周期，只下发失败或未完成的阶段。威胁分析续扫先以 `is_resume=true` 复用已有阶段产物；若原生入口明确返回失败且任务未取消，外层协调器会归档旧产物并只做一次干净重跑。其它已经成功的引擎不会重复执行；消费威胁分析产物的 `threat_pattern_audit` 与历史 `threat_audit` 会在对应阶段需要恢复时等待威胁分析成功后补跑。多版本组合引擎还会在扫描工作目录中逐版本保存已成功的静态候选结果，续扫直接复用完整或空结果，只重跑未完成、缺失或损坏的版本。
...
项目总路径和非空的扫描模块路径必须填写**目标 Agent 上的绝对路径**，例如 `/home/user/project/src`、`C:\work\project\src` 或 `\\server\share\project\src`；不接受 `src`、`./src`、`../src`、`~/project`、`C:src` 等写法。扫描模块路径仍可留空，表示扫描项目总路径。页面提交时会提示具体错误字段，创建扫描接口也会拒绝相对路径；多版本模式对每个版本分别校验。目录是否存在、扫描目录是否位于项目内仍由 Agent 检查。
...
- **恢复**：在扫描列表页点击「恢复」，服务器通知 Agent 继续同一扫描任务，自动跳过已处理的候选；已经成功并保存合法结果的威胁分析、攻击模式审计及多版本模型任务直接复用，失败或缺少结果的单项任务以新 Session 重跑。无需重新启动 Agent 或重新索引代码。
...
- 文件、SKILL 与 `bash` 的稳定权限统一写入 Serve 实际使用的全局 `opencode.json`。最终配置全局允许 `read`、`list`、`glob`、`grep` 和 `external_directory`，因此 OpenCode 可读取运行账户有权访问的全部本地路径，包括项目目录之外的真实版本仓库、隐藏文件和配置文件，不再依赖每个 Session 逐项声明项目路径。该变更不扩大编辑权限：`edit` 仍默认拒绝，`work_dir` 默认可写并自动进入保留白名单，完整 Agent 继续只为 `~/.opendeephole/scans`、`fp_reviews`、`vulnerability_validation`、`skill_create` 及显式动态白名单保留原有写入规则，`bash` 仍全面禁用。`file_write_allowlist` 和兼容参数 `writable_paths` 会合并为额外可写并默认保留的动态路径，每次调用都替换当前 Session 的路径覆盖，避免续接时残留旧权限。脱离 Agent 运行时会从合并后的全局、项目、环境显式配置及 `task-agent.yaml` 中加载 `skills.paths`，任务级 `skill_paths` 继续追加且不获得编辑权限。
...
| 位置 | 内容 | 生命周期 |
|------|------|---------|
| `<扫描路径>/code_index.db` | 静态分析使用的 tree-sitter 代码索引（函数/结构体/调用关系） | 仅选择静态分析引擎时创建，后续相同范围扫描复用 |
| `~/.opendeephole/scans/<scan_id>/` | 扫描工作目录（candidates.json、多版本阶段检查点、config.yaml、agent.log 等） | 扫描成功后自动删除；取消/出错时保留用于恢复 |
...
> **注意：** `code_index.db` 直接保存在本次 `code_scan_path` 指向的扫描目录下，且只索引该目录。扫描子目录时不会读取或删除项目根目录遗留的旧索引；扫描整个项目时两者是同一路径。对于大型扫描范围，该文件可能有几十到几百 MB，删除后会在下次选择静态分析引擎时自动重建。
...
│   │   │   │   ├── engine.yaml
│   │   │   │   └── engine.py       # 平台引擎适配器
│   │   │   └── threat_pattern_audit/ # 按高风险模块与攻击模式执行的审计引擎
