...
**失败续扫**：服务端根据独立威胁分析和各漏洞挖掘引擎的持久化生命周期，只下发失败或未完成的阶段。威胁分析续扫先以 `is_resume=true` 复用已有阶段产物；若原生入口明确返回失败且任务未取消，外层协调器会归档旧产物并只做一次干净重跑。其它已经成功的引擎不会重复执行；消费威胁分析产物的 `threat_pattern_audit` 与历史 `threat_audit` 会在对应阶段需要恢复时等待威胁分析成功后补跑。
