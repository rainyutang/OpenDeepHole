...
**扫描流程与威胁分析选择**：扫描详情主流程保持“威胁分析 → 漏洞挖掘 → 漏洞验证”，去误报从漏洞挖掘之后并列分出，代码图谱构建作为底层能力单独显示；静态分析归入“DeepHole基于代码风险点的漏洞挖掘引擎”的内部阶段。新建扫描通过 `threat_analysis_enabled` 独立选择威胁分析，并通过 `threat_analysis_method` 从 `deephole_client/threat_analysis/methods/` 自动发现的方法中选择一个；内置方法为“DeepHole威胁分析”。`mining_engines` 独立选择“DeepHole基于代码风险点的漏洞挖掘引擎”和“DeepHole基于攻击威胁的漏洞挖掘引擎”；可以只运行威胁分析，但选择后者时必须同时启用威胁分析。代码图谱完成后，威胁分析与无需其产物的挖掘工作仍可并行，攻击威胁引擎等待威胁分析成功后再启动，验证与去误报继续按确认问题增量触发。
...
- 所有模型任务只通过 OpenCode 兼容的 serve API 调用，并严格启动配置的 executable，不会因为同时安装了 `opencode` 而回退到它。Agent 优先使用 `base.opencode_serve_port`，未配置时兼容 `OPENCODE_SERVE_PORT`，两者都没有时由操作系统分配一个空闲端口；自动端口在同一 Agent 进程内跨 Serve 重启复用，若启动前已被其它进程占用、无监听却无法绑定，或首次只返回泛化的 `Error: Unexpected error`，会最多尝试 3 个不同端口且不终止未知监听者；显式端口永不自动改号，并在占用、Windows 排除/保留端口或端点安全软件拒绝时直接报告监听 PID 或绑定错误。两种模式都只会回收已通过归属标记和进程树证明属于本 Agent 的进程；POSIX 退出时先尝试 `SIGTERM`，超时后以 `SIGKILL` 清理整个受管进程组及已登记监听 PID。Windows 仍以 ownership marker 与目标端口监听 PID 的交集限定 `taskkill /T /F` 目标，但最终以 TCP 是否可连接及 `SO_EXCLUSIVEADDRUSE` 独占绑定能否成功判断端口是否真的阻塞新 Serve：PID 仍出现在监听表但端口已可独占绑定时按幽灵记录处理并删除旧 marker，端口不可绑定时继续保留 marker 和管理器状态供重试。旧受管 Serve 未确认清理完成时不会换端口或启动新进程，同一 Agent 进程最多登记一个 Serve。Linux/macOS 启动脚本以 `exec` 运行 Agent，使发给脚本 PID 的退出信号直接进入 Agent 清理流程。Agent 重启后会重新选择自动端口；配置更新在活动 Session 结束后的安全重启边界生效。standalone `task-agent.yaml` 继续使用显式 `serve.port`（默认 `4096`）。组件只调用 `task_agent.run_opencode_task()`；真实项目目录和 `.opendeephole` 工作目录由执行上下文提供，不回退到当前目录。调用方不能传原生 permission；推荐通过 `file_write_allowlist` 显式增加路径级写权限，`writable_paths` 作为兼容别名保留。
