...
**两种扫描方式**：① **候选点扫描**——静态分析器找候选点 + AI 逐个审计（按检查项）；② **深度挖掘**（独立流程，新建扫描页可切换）——多 Agent 自主挖掘漏洞，不依赖静态分析器预设模式：**威胁分析 Agent** 先扫描测试代码路径（可参考创建时上传的文档）识别入口函数 → 每个入口起一个**挖掘 Agent** 深挖（发现新攻击面则派生新挖掘 Agent）→ 发现的问题立即以"待验证"显示并派生**验证 Agent** 判定是/非问题 → 入口分析完后按**函数覆盖率**对未覆盖函数继续挖掘，以「覆盖完成或 Agent 调用上限」双闸停止。深挖有专属多 Tab 详情页（概览/Agent 任务/发现/日志），实时展示进行中/即将进行的任务、覆盖率与预算、发现漏洞，并可点开任一 Agent 查看其实时与最终输出。  
...
- 排队数只统计真正在等待的任务：有空闲模型时立即获取，不产生排队计数；排队目标按 `(排队数+运行数)/(weight×max_concurrency)` 选择预期最快空出槽位的模型。
- 「最近状态」按实时计数派生：运行中 > 排队 > 最近一次结果（成功/失败/超时/取消），空闲且无历史时显示 "-"。
